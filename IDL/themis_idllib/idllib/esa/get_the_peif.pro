...
FUNCTION get_the_peif,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times, $
                      bgnd_remove=bgnd_remove,bgnd_type=bgnd_type,bgnd_npoints=bgnd_npoints,bgnd_scale=bgnd_scale
...
;
; remove background contamination from penetrating electrons
;
if (keyword_set(bgnd_remove)) then begin
  if (keyword_set(bgnd_scale) eq 0) then bgnd_scale=1
  if (keyword_set(bgnd_type) eq 0) then bgnd_type='angle'
  if (keyword_set(bgnd_npoints) eq 0) then bgnd_npoints=1
  gfeff = gf*eff
  narrdims=size(dat,/dimensions)
  case bgnd_type of
  'angle' : bgnd= gfeff * (make_array(nenergy,value=1,/float) # $
            minjmin(eff*dat/gf,dim=1,jmin_points=bgnd_npoints)) ;  2d or 1d
  'omni' : begin
             if (n_elements(narrdims) eq 1) then bgnd=gfeff * $
               minjmin(eff*dat/gf,jmin_points=bgnd_npoints)
             if (n_elements(narrdims) eq 2) then bgnd=gfeff * $
               minjmin(average(eff*dat/gf,2),jmin_points=bgnd_npoints)
           end
  'anode' : begin
             if (n_elements(narrdims) eq 1) then bgnd=gfeff * $
               minjmin(eff*dat/gf,jmin_points=bgnd_npoints)
               ...
               for ith=0,nths-1 do begin 
                 ...
                 if jan gt 0 then begin
                   if jan eq 1 then begin
                    bgnd(*,ian)=gfeff(*,ian) * $
                     minjmin(eff(*,ian)*dat(*,ian)/gf(*,ian),jmin_points=bgnd_npoints)
                   endif else begin
                     bgnd(*,ian)=gfeff(*,ian) * $
                     minjmin(average(eff(*,ian)*dat(*,ian)/gf(*,ian),2),jmin_points=bgnd_npoints)
                   endelse
                 endif
               endfor
             endif
           end
  else:    print,'unknown bgnd_type entered'
  endcase
