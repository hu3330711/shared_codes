pro thm_filter_test,tplot_name_init,period_long,period_short
...
print,'check nan'
  ...
  dt=find_datarate(data.x(97:99))
  endif else begin
  wdw=ceil(period_long/find_datarate(data.x(2:4)))
  dt=find_datarate(data.x(2:4))
  endelse

  print,'wdw=',wdw

  if(wdw gt n_elements(data.x)) then return
  ;; for xx=0,dimension_input-1 do data_s [*,xx]=filter_test(data.y [*,xx],wdw,/LOWPASS)
for xx=0,dimension_input-1 do data_b [*,xx]=thm_lsp_filter_fft(data.y[*,xx],dt,1./period_long,1./period_short)
for xx=0,dimension_input-1 do data_s [*,xx]=thm_lsp_filter_fft(data.y[*,xx],dt,0,1./period_long)
for xx=0,dimension_input-1 do data_bd [*,xx]=thm_lsp_filter_fft(data.y[*,xx],dt,0,1./period_short)
for xx=0,dimension_input-1 do data_d [*,xx]=data.y [*,xx]-data_bd [*,xx]
  ...
  ;; for xx=0,dimension_input-1 do data_b [*,xx]=filter_test(data_bd[*,xx],wdw,/LOWPASS)
  ;; for xx=0,dimension_input-1 do data_b [*,xx]=butterworth(data_bd[*,xx],wdw,order=3)
