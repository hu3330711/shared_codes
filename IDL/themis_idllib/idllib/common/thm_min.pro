...
pro thm_min,tplot_name_init,period,onemed=onemed,ulimit=ulimit,llimit=llimit
...
for loop=0,n_elements(tplot_name_init)-1,1 do begin
  ...
  for xx=0,dimension_input-1 do begin
    if not keyword_set(onemed) then begin
      data_m [*,xx]=min(data.y [*,xx],wdw,/nan)
      data_d [*,xx]=data.y[*,xx]-data_m[*,xx]
    endif else begin
        data_m [*,xx]=min(data.y [*,xx],dimension=1,/nan)
        data_d [*,xx]=data.y[*,xx]-data_m[*,xx]
    endelse
  endfor
  ...
  store_data,tplot_name_init(loop)+'_min' ,data=data,dlim=dlim,lim=lim2
  str_element,data,'y',data_d,/add_replace
  store_data,tplot_name_init(loop)+'_mind',data=data,dlim=dlim,lim=lim3
endfor
