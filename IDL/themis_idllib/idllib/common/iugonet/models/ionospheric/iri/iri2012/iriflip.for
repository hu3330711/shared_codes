...
C...      EF=RTS(61)*0.76/L1
C      IF(JPT.EQ.1.AND.JPR.GT.0.AND.INT(EF+0.1).NE.1) WRITE(I,189)
C      IF(JPT.EQ.1.AND.JPR.GT.0.AND.INT(EF+0.1).EQ.1) WRITE(I,191)
C 189  FORMAT(/2X,'N(2D)',25X,'EMISSION',28X,':',20X,'Loss rate')
C 191  FORMAT(/2X,'N(2D)',25X,'Production',36X,':',20X,'Loss rate')
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,96)
C 96   FORMAT(2X,'ALT   [N2D]   NO++e   N2++e   N2++O    e+N2   hv+N2'
C     >  ,3X,'N++O2   N(2P)   N2A+O    +O     +O2      +e     +O+'
C     >  ,5X,'RAD     +NO')
C      IF(JPR.GT.0) WRITE(I,7) Z,P1/L1,(PR(K)*EF,K=1,8)
C     > ,(LR(K)*N2D,K=1,6)
      RETURN
C 7    FORMAT(F6.1,1P,22E8.1)
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C........no
      ...
      L1=LR(1)+LR(2)+LR(3) + LR(4) + (LR(5)+LR(6))
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,192)
C 192   FORMAT(/2X,'NO',17X,'PRODUCTION',20X,':',10X,'LOSS RATES'/
C     > ,4X,'ALT',3X,'[NO]',5X,'[NO]c',3X,'O2+N2D',
C     > 3X,'O2+N4S   N2P+O2   N2A+O    N++O2    N4S+NO   O2P+NO   O++NO'
C     > ,3X,'N2D+NO   hv<1910   Lyman-a')
C      IF(JPR.GT.0) WRITE(I,7) Z,NNO,P1/L1,(PR(K),K=1,5)
C     > ,(LR(K)*NNO,K=1,6)
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,193)
C 193   FORMAT(/2X,'N(4S)',38X,'PRODUCTION',46X,':',7X,'LOSS RATES'/
C     > ,3X,'ALT',2X,'[N4S]',2X,'hv->N+'
C     > ,3X,'O+N2D',2X,'e+N2D',3X,'O++N2',3X,'N2++e',4X,'hv->2N'
C     > ,2X,'NO++e',2X,'N(2D)',4X,'N(2P)   N+&X    hv+NO    +O2  '
C     > ,2X,' +NO  ',2X,'+O2+ & N2+')
      PR(10)=PR(10)+PR(12)  !... for printing fit
      LR(3)=LR(3)+LR(4)     !... for printing fit
C      IF(JPR.GT.0) WRITE(I,7) Z,N4S,(PR(K),K=1,11),(LR(K)*N4S,K=1,3)
      RETURN
C 7    FORMAT(F6.1,1P,22E8.1)
      END
C::::::::::::::::::::::::::::::: CN2PLS :::::::::::::::::::::::::::::::
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,95)
C 95   FORMAT(/2X,'N2+',29X,'PRODUCTION',45X,':',12X,'LOSS RATES'/
C     > ,3X,'ALT  [N2+]  EUV-X   EUV-A    EUV-B   PE-X'
C     > ,5X,'PE-A    PE-B  O+2D+N2  O+2P+N2  He++N2  O+N2+'
C     > ,2X,'e+N2+  O2+N2+  N2++O  Other')
      PR(9)=PR(9)+PR(10)     !.. for printing fit
      LR(5)=LR(5)+LR(6)      !.. for printing fit
C      IF(JPR.GT.0) WRITE(I,7) Z,N2PLS,(PR(K),K=1,9)
C     > ,(LR(K)*N2PLS,K=1,5)
      RETURN
C 7    FORMAT(F6.1,1P,22E8.1)
      END
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,96)
C 96   FORMAT(/2X,'NO+',31X,'PRODUCTION',48X,':',2X,'LOSS RATES'/
C     > ,3X,'ALT',3X,'[NO+]',4X,'O++N2',3X,'N2++O',3X,'O2++N4S'
C     > ,3X,'O2++NO',3X,'N++O2',4X,'N2P+O',3X,'O++NO   hv+NO'
C     > ,5X,'O2++N2D   N2++NO   N++NO   OP2D+NO   OP2D+N2  NO++e')
      !PR(9)=PR(9)+PR(10)+PR(11)+PR(12)+PR(13)
C      IF(JPR.GT.0) WRITE(I,7) Z,NOP,(PR(K),K=1,13),LR(1)*NOP
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,97)
C  97  FORMAT(/2X,'O2+',22X,'PRODUCTION',24X,':',12X,'LOSS RATES'
C     > /,3X,'ALT',3X,'[O2+]',3X,'hv+O2',3X,'O++O2',3X,'O+(2D)+O2'
C     >  ,4X,'N2++O2   N++O2   O+(2P)+O2  O2++e   O2++N   O2++NO')
C      IF(JPR.GT.0) WRITE(I,7) Z,O2P,(PR(K),K=1,7),(LR(K)*O2P,K=1,3)
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C.........pr(1)= euv production of o+(4s)
      ...
      OPLS=PRTOT/LRTOT
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,98)
C 98   FORMAT(/2X,'O+',41X,'PRODUCTION',39X,':',10X,'LOSS RATES'/
C     >  ,' ALT    [O+]   hv+O  O+(2D)+e O+(2P)+O   e*+O  O2-diss  '
C     >  ,'N2++O  O+(2P)+e O+(2P) O+O+(2D)   Other  +N2     +O2    '
C     >  ,'+NO   +N2D')
C      IF(JPR.GT.0) WRITE(I,7) Z,OPLS,(PR(K),K=1,10)
C     > ,(LR(K)*OPLS,K=1,4)
      RETURN
C 7    FORMAT(F6.1,1P,22E8.1)
      END
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,99)
C 99   FORMAT(/2X,'O+(2D)',13X,'PRODUCTION',27X,':',18X,'LOSS RATES'/
C     > ,3X,'ALT',3X,'[O+2D]',3X,'hv+O',4X,'e*+O',4X,'O+2P+e',3X,
C     > 'O+2P>hv',2X,'He++O2     +N2    E3726_29    +e       +O',
C     > '      +O2      +NO     +N  +N2>NO+')
C      IF(JPR.GT.0) WRITE(I,7) Z,OP2D,PR(1),PR(5),PR(2),PR(3),PR(4)
C     > ,(LR(K)*OP2D,K=1,8)
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
...
C.......o+(2p)
      ...
      OP2P=(TPROD3+PR(3))
     >  /(LR(1)+LR(2)+LR(3)+LR(4)+LR(5)+LR(6)+LR(7)+LR(8))
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,100)
C 100   FORMAT(/2X,' O+(2P)',6X,'PRODUCTION',10X,':',12X,'LOSS RATES'/
C     >,3X,'ALT   [O+2P]    hv+O     e*+O  He++O2      +O',7X,'+N2'
C     > ,6x,'+e       RAD      +e      +O2      +N4S     +NO'
C     > ,6x,'OX       N2        e      Te       E7320')
C      IF(JPR.GT.0) WRITE(I,7) Z,OP2P,PR(1),PR(2),PR(3),
C     >  (LR(K)*OP2P,K=1,8),ON,N2N,NE,TE,OP2P*0.218*0.781
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,101)
C 101   FORMAT(/2X,'N+',20X,'PRODUCTION',71X,':',8X,'LOSS RATES'/
C     > ,4X,'ALT   [N+]   [N+]c     hv+N2   O++N2D  O+2P+N2',3X
C     > ,'He++N2',3X,' hv+N   O2++N2D  N2++N4S O+(2D)+N4S  He++NO'
C     > ,3X,'N++O2    N++O2    N++O2    N++O2    N++O2    N++O')
C      IF(JPR.GT.0) WRITE(I,7) Z,NPLUS,CNPLUS
C     > ,(PR(K),K=1,9),(LR(K)*NPLUS,K=1,6)
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,102)
C 102   FORMAT(/2X,'N2(A)',12X,'PRODUCTION',13X,':',5X,'LOSS RATES'
C     > ,3X,':  Total LBH'
C     > /,3X,'ALT',3X,'N2(A)',3X,'e*->N2A',3X,'e*->N2B',3X,'e*->N2C',2X
C     >  ,'N2A>O1S',2X,'N2A>NO',2X,'RAD',5X,'LBH')
C      IF(JPR.GT.0) WRITE(I,7) Z,N2A,(PR(K),K=1,3),(LR(K)*N2A,K=1,3)
C     > ,P3X4
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
...
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,103)
C 103   FORMAT(/2X,'N(2P)',9X,'PRODUCTION',17X,':',20X,'LOSS RATES'/
C     > ,3X,'ALT',3X,'[N2P]',3X,'e+N2',5X,'hv+N2',3X,'e+N2+',6X,'+O  '
C     > ,3X,'+O2      +O2+      +NO       +2D     +4S      +e')
C      IF(JPR.GT.0) WRITE(I,7) Z,N2P,(PR(K),K=1,3),(LR(K)*N2P,K=1,7)
      RETURN
C 7    FORMAT(F6.1,1P,22E9.2)
      END
C:::::::::::::::::::::::::::::: CNOPV ::::::::::::::::::::::::::::::::::
...
C          IF(JPR.GT.0.AND.Z.GE.0.AND.Z.LT.10)
C     >     WRITE(6,'(F10.1,I7,1P,22E10.2)') 
C     >     Z,IV,PNOPV,P1,PCASC,P_N2_Q,LNOPV,LRAD,L_N2_Q,
C     >     NOPV(IV),NOP,NOPTOT
      ENDDO

C      PR(9)=PR(9)+PR(10)+PR(11)+PR(12)         !.. for printing only
C      IF(JPT.EQ.1.AND.JPR.GT.0) WRITE(I,96)
C 96   FORMAT(/5X,'NO+',34X,'PRODUCTION',69X,':LOSS RATE'/
C     > ,3X,'ALT NO+(v=0) NO+(v=1) NO+(v=2)  NO+(v=3) O++N2    N2++O',
C     >  4X,'O2++N4S   O2++NO   N++O2    N2P+O   O++NO   hv+NO',
C     >  4X,'Other_P   NO++e')
C     IF(JPR.GT.0) WRITE(I,'(F6.1,1P,22E9.2)')  Z,
C    >   (NOPV(K),K=1,4),(PR(K),K=1,9),LR(1)*NOP
...
C        IF(Y.GT.100.0) WRITE(6,100) I,Z/1.0E5,CHI*57.3,TNJ,EM,M(I),GR,RP
C 100    FORMAT('WARNING, Y IN COLUMN(I) > 100',I4,1P,9E10.2)
...
C
      ...
      NNI(3)=6
      LMAX=37
C      IF(ISW.NE.0) WRITE(17,95)
C 95   FORMAT(/5X,'EUV fluxes, Photoabsorption, and Photoionization ',
C     >  'Cross sections',
C     > /4X,'I',5X,'lam',5X,'flux',4X,'sigaOX',3X,'sigaO2'
C     > ,3X,'sigaN2',3X,'sigiOX',3X,'sigiO2',3X,'sigiN2',3X,'UVfac')
C
      DO 20 L=1,LMAX
      ZLAM(L)=ZLX(L)
      FFAC=UVFAC(LMAX+1-L)
...
C      WRITE(17,90) L,ZLAM(L),ZFLUX(L),(SIGABS(I,L),I=1,3)
C     > ,(SIGION(I,L),I=1,3),FFAC
 20   CONTINUE
C
      IF(ISW.EQ.0) RETURN
C      WRITE(17,94)
C 94   FORMAT(/5X,' Ionization potentials for O, O2, N2'
C     > ,/2X,'4S   2D   2P   4P   2P*  -   X2   a+A  b4   B2   dis  -'
C     > ,'  X2   A2   B2   C2   F2   2s')
C 60   WRITE(17,91) ((TPOT(I,J),J=1,6),I=1,3)
C
      RETURN
C 90   FORMAT(1X,I4,F9.2,1P,22E9.2)
C 91   FORMAT(22F5.1)
      END
C:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
C
      IF(ISW.EQ.0) RETURN
C      WRITE(17,95)
C 95   FORMAT(/5X,' Photoionization branching ratios for O, O2, N2'
C     > ,/3X,'Lam    4S   2D   2P   4P   2P*   -   X2   a+A  b4   B2 '
C     > ,'  dis   -  X2   A2   B2   C2   F2   2s')
C      DO 50 L=1,LMAX
C 50   WRITE(17,90) ZLAM(L),((PROB(IS,J,L),J=1,6),IS=1,3)
C 90   FORMAT(F8.2,22F5.2)
       RETURN
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
