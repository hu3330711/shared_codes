...
c
1      jjjjj=0
c       print *,'jmag(=0/1,geog/geom),lati/deg,long/deg'
        read(5,*) jm,xlat,xlon
...
c         print *,'Compute Ne, T, Ni? (enter: t,t,t  if you want all)'
          read(5,*) jf(1),jf(2),jf(3)
        if(jf(1)) then 
c             print *,'LAY version: t=standard ver., f=LAY version.',
c     &              ' {standard:t}'
              read(5,*) jf(11)
c             print *,'Ne Topside: t=IRI-2001/h0.5, f=new options {f}'
              read(5,*) jf(29)
c             print *,'Ne Topside: t=IRI01_corrt, f=NeQuick/h0.5 {f}'
              read(5,*) jf(30)
c             print *,'Ne Topside: t=F10.7<188, f=unlimited {t}'
              read(5,*) jf(7)
c             print *,'Ne topside: t=w/o foF2 storm model, f=with {t}'
              read(5,*) jf(37)
c             print *,'foF2 model: t=CCIR, f=URSI-88 {f}'
              read(5,*) jf(5)
c             print *,'foF2: t=with storm model, f=without {t}'
              read(5,*) jf(26)
c             print *,'hmF2: t=f(M3000F2), f=new models {f}'
              read(5,*) jf(39)              
c             print *,'hmF2: t=AMTB-model, f=Shubin-COSMIC model {t}'
              read(5,*) jf(40)              
              if(jf(39)) then
c                print *,'hmF2: t=w/o foF2 storm model, f=with {t}'
                 read(5,*) jf(36)
                 endif              
c             print *,'F2 peak density or foF2: t=model, ',
c    &              'f=user input {t}'
              read(5,*) jf(8)
c             print *,'F2 peak height or M3000F2: t=model, ',
c    &              'f=user input {t}'
              read(5,*) jf(9)
c             print *,'Auroral boundary model: t=on, f=off {f}'
              read(5,*) jf(33)
c             print *,'Bottomside thickness B0: t=Bil-2000, ',
c    &            'f=other options {f}.'
              read(5,*) jf(4)
c             print *,'Bottomside thickness B0: t=ABT-2009, ',
c    &            'f= Gul-1987 {t}.'     
              read(5,*) jf(31)
c             print *,'Bottomside thickness B0: t=model, ',
c    &            'f=user input {t}.'     
              read(5,*) jf(43)
c             print *,'F1 peak density or foF1: t=model, ',
c    &            'f=user input {t}'
              read(5,*) jf(13)
            if(.not.jf(11)) then
c             print *,'F1 peak height: t=model, f=user input {t}'
              read(5,*) jf(14)
            endif
c             print *,'F1: t=with probability model, f=without   {t}'
              read(5,*) jf(19)
c            print *,'F1: t=standard probability, f=with L ',
c    &            'condition {t}'
              read(5,*) jf(20)
c             print *,'E peak density or foE: t=model, f=user input {t}'
              read(5,*) jf(15)
c            print *,'E peak height: t=model, f=user input {t}'
              read(5,*) jf(16)
c            print *,'E peak auroral storm model: t=on, f=off {f}'
              read(5,*) jf(35)
c             print *,'D: t=IRI-1990, f= FT-2001 {t}'
              read(5,*) jf(24)
          endif
        if(jf(2)) then
c             print *,'Te(Ne) model: t=not used, f=correlation is',
c   &          ' used. {t}'
              read(5,*) jf(10)
c             print *,'Te: t=Bil-1985, f=TBT-2012 {f}'
              read(5,*) jf(23)
c            print *,'Te: t=TBT-2012 with PF107 dep., f=w/o {t}'
              read(5,*) jf(42)
          endif
        if(jf(3)) then
c             print *,'Ion comp. model: t=DS95/DY85, f=RBV10/TTS05 {f}' 
              read(5,*) jf(6)
              if(.not.jf(6)) then
c            	print *,'IRIFLIP: t=messages off, f=on {t}' 
              	read(5,*) jf(38)
              	endif              
c            print *,'Ni: t=ion composition in %, f=ion densities',
c   &             'in cm-3 {t}'
              read(5,*) jf(22)
              endif
c          print *,'Equat. Vert. Ion Drift: t=computed, ',
c    &            'f=not computed {t}'
              read(5,*) jf(21)
c         print *,'Spread-F probability: t=computed, ',
c    &            'f=not computed {t}'
              read(5,*) jf(28)
c          print *,'COV: t: COV=F10.7_365, f: COV=func(IG12).  {t}'
              read(5,*) jf(41)
c          print *,'Sunspot index: t=from file, f=user input.  {t}'
              read(5,*) jf(17)
c          print *,'Ionospheric index: t=from file, f=user input. {t}'
              read(5,*) jf(27)
c         print *,'F10.7D Index: t=from file, f=user input {t}'
              read(5,*) jf(25)
c         print *,'F10.7_81 Index: t=from file, f=user input {t}'
              read(5,*) jf(32)
c          print *,'dip, magbr, modip: t=IGRF, f=old FIELDG using ',
c   &             'POGO68/10 for 1973 {t}'
              read(5,*) jf(18)
c         print *,'Messages on (t) off (f) {t}'
              read(5,*) jf(34)
c          print *,'Message output unit: t=(UNIT=6), f=(UNIT=11). {t}'
              read(5,*) jf(12)
       endif
...
c
      ...
      if(PIKTAB.eq.3) then
c       print *,'6 Parameters of your choice (number:1-86)'
c        print *,(pna(j),j=1,10)
...
c        print *,(pna(j),j=81,86)
c        print *,'or 0,0,0,0,0,0 for default:'
c        print *,'      spread-F probability [48]'
c        print *,'      equatorial vertical ion drift [44]'
c        print *,'      foF2_storm/foF2_quiet [45]'
c        print *,'      foE_storm/foE_quiet [47]' 
c        print *,'      eqward auroral boundy CGM-Lat [58]'
c        print *,'      solar zenith angle [23]'
c        print *,'      modified dip latitude [27]' 
c        print *,'      Ap for current UT [51]' 
        read(5,*) (pad1(j),j=1,6)
...
c              print *,'foF2/Mhz or NmF2/m-3 for ',itext(ivar),
c     &             '=',var
              read(5,*) oar(1,i)
              pname(1)='foF2/MHz'
            ...
            if(.not.jf(9)) then
c              print *,'hmF2/km or M3000F2 for ',itext(ivar),
c     &              '=',var
              read(5,*) oar(2,i)
              pname(2)='M(3000)F2'
            ...
            if(.not.jf(13)) then
c              print *,'foF1/MHz or NmF1/m-3 for ',itext(ivar),
c     &               '=',var
              read(5,*) oar(3,i)
              pname(3)='foF1/MHz'
...
c              print *,'hmF1/km for ',itext(ivar),'=',var
            ...
            if(.not.jf(15)) then
c              print *,'foE/MHz or NmE/m-3 for ',itext(ivar),
c     &                '=',var
              read(5,*) oar(5,i)
              pname(5)='foE/MHz'
            ...
            if(.not.jf(16)) then
c              print *,'hmE/km for ',itext(ivar),'=',var
              read(5,*) oar(6,i)
              pname(6)='hmE/km'
              endif
            if(.not.jf(43)) then
...
c
        if(jf(2).and..not.jf(10)) then
          var=vbeg
          do 1235 i=1,numstp 
c            print *,'Ne(300km),Ne(400km)/m-3',
c     &         ' for ',itext(ivar),'=',var,' [-1 if not]'
...
c                        print *,'User input for F10.7D'
            ...
            if(.not.jf(32)) then
c                        print *,'User input for F10.7_81d'
                        read(5,*) oar(46,1)
                        do i=2,100
                                    oar(46,i)=oar(46,1)
                                    enddo
