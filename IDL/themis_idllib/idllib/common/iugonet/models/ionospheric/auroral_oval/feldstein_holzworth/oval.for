...
1	type *,'mlt,iq(0-6)'
	read(*,*) xmlt,iq
	if(xmlt.lt.0.0) goto 2

	call oval(xmlt,iq,pcgl,ecgl)

	type *,'mlt=',xmlt,' IQ=',iq,'  polw.:',pcgl,'  equatorw.:',ecgl
...
C			to active (6)
C output:	PCGL	corrected geomagnetic latitude of ploward
C			boundary
...
C --------------------------------------------------------------------- 
	...
	iq = iql+1
	PHI = XMLT *15.
	type *,umr,iq,xmlt,phi
	type *,a1(iq),a2(iq),a3(iq),a4(iq),a5(iq),a6(iq),a7(iq)
	...
	type *,umr,iq,xmlt,phi
	type *,b1(iq),b2(iq),b3(iq),b4(iq),b5(iq),b6(iq),b7(iq)
