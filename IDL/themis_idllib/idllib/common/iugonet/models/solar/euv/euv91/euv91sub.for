C EUV91SUB.FOR ------------------------------------------------ Feb 6, 1992
C
C Includes subroutines euv91, rddata, flux, getind and block data
C
        subroutine euv91
C********1*********2*********3*********4*********5*********6*********7**
...
C NSSDC --- NSSDC --- NSSDC --- NSSDC --- NSSDC --- NSSDC --- NSSDC ----
C
C Adopted for NSSDC and distribution / Dieter Bilitza / NSSDC-Id: MS-13D
C       6/2/92   COMMON before DATA statements
C       6/2/92   units and dates in output file
C       6/2/92   get fname without do loop
C       6/2/92   BLOCK DATA statement for coefficients in RDDATA
C ______________________________________________________________________
C********1*********2*********3*********4*********5*********6*********7**
...
C********1*********2*********3*********4*********5*********6*********7**
          ...
          do 30 io=3,6,3
          write (io,*) '   EUV 91 (4/30/91)          Date:',yyddd+i
          write (io,*) '   Wavelength    Photon flux      Energy flux'
          write (io,*) '    Angstrom       cm-2 s-1      erg cm-2 s-1'
          do 30 j=1,39
            write(io,'(x,2(F7.2,x),2x,2(E9.3,8x))')
...
10      continue
        ...
        open (unit=1,file='euv91coe.txt',status='old',form='formatted')
        do 20 i = 1,3
          read(1,'(A)') line1
20      continue
C
        do 30 i = 1,52
          ...
          number = 0.
          sign = 1
          read (1,3311) itemp(1),itemp(2),itemp(3),c(4,i),line2
3311      format(I4,1x,I4,1x,I1,1x,G12.5E4,1x,A51)
          c(1,i) = dble(itemp(1))
          c(2,i) = dble(itemp(2))
          c(3,i) = dble(itemp(3))
          do 40 j = 1,51
...
C********1*********2*********3*********4*********5*********6*********7**
      open(unit=2,file='euv91ix2.dat',status='old',form='formatted')
        do 50 i = 1,2
          read(2,'(A)') line1
50      continue
        do 60 i = 1,3750
          read (2,4455) itemp(1),indice(2,i),indice(3,i),itemp(2),
     3  itemp(3),itemp(4),indice(7,i),indice(8,i),itemp(5),itemp(6)
4455      format(1x,I5,2(1x,G11.5E4),2I4,3x,I5,2(1x,G11.5E4),2I4)
          indice( 1,i) = dble(itemp(1))
          indice( 4,i) = dble(itemp(2))
          indice( 5,i) = dble(itemp(3))
          indice( 6,i) = dble(itemp(4))
          indice( 9,i) = dble(itemp(5))
          indice(10,i) = dble(itemp(6))
60      continue
        close (unit=2)
...
C********1*********2*********3*********4*********5*********6*********7**
        ...
        do 10 j=1,52
          sumflx = 0.
          do 20 i=1,4
            if (i .le. 2) exparg = -S(i)*1.e-10
            if (i .gt. 2) exparg = -S(i)*.7
            if (exparg .lt. -argmax) exparg=-argmax
            W(j) = (1.+c(8+i,j)*exp(exparg))*W(j)
...
C
        BLOCK DATA
        real            c(12,52),indice(10,3750),S(4)
        real            date,Eflux(39),Pflux(39)
        real            w1(39),w2(39),wave(39)
        common  /coeff/ c,indice,date,wave,w1,w2,S,Eflux,Pflux
C********1*********2*********3*********4*********5*********6*********7**
