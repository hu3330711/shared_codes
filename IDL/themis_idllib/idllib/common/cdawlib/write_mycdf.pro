...
case c(nc-2) of
  ...
  9   : begin
          dtype(14) = 1L
          ;print,'WARNING>CDF does not have double_complex_float type'
	end
...
; NAME: WRITE_MYCDF
; PURPOSE:
;       To input up to 10 idl structures of the type returned by read_mycdf,
;       and to output each as a CDF file.
; CALLING SEQUENCE:
;       status = write_mycdf(a0,a1,a2,a3,a4,a5,a6,a7,a8,a9)
; INPUTS:
...
FUNCTION write_myCDF, a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,$
...
for k=0,n_params()-1 do begin ; process each structure parameter
   ...
   if q[0] eq -1 then begin
	...
	if strupcase(g) eq 'LOGICAL_SOURCE' then begin
           if (n_elements(lsrc) gt 3) then $
           a.(0).(gattrs(i))=strupcase(lsrc[0]+'_'+lsrc[1]+'s_'+lsrc[2]+ $
                 '_'+lsrc[3]) else $
	   a.(0).(gattrs(i))=strupcase(lsrc[0]+'_'+lsrc[1]+'s_'+lsrc[2])
	endif   
	...
	if strupcase(g) eq 'TEXT' then begin
	   ...
	   'Tami.Kovalick@gsfc.nasa.gov, Rita.Johnson@gsfc.nasa.gov. ' + $ 
	      a.(0).(gattrs(i))
	endif   
	;
