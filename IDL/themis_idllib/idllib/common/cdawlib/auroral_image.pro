...
;
;  $Id: auroral_image.pro 8 2006-10-11 20:32:51Z kenb $
;
...
if keyword_set(centerPole) then begin ; define at closest pole
   ...
   if keyword_set(rangeLonLat) then $  ;switched for S.Pole, otherwise continents don't show
...
if (method1 ne "TV") and (method1 ne "QU") then begin
   if not keyword_set(nogrid) then begin 
	 ...
	 ; removed the latdel=10 from the call to map_grid.
         map_grid,color=mapColor,charsize=mapCharSize,_Extra=extra
      endelse
   endif
