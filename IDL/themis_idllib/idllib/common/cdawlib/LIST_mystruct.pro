...
;
; PURPOSE: Reforms the data array from a (i,j,k) to a (i*j,k).
;
; astrc    -  Input structure
      ...
      for j=0, ntags-1 do begin
         if(names(j) eq 'DAT') then begin
            ...
            if(not execute(tag_cmd)) then begin
               print, 'ERROR=Execute error setting tag_cmd2'
               close,1
               return, -1
            endif
            if(j eq 0) then begin
   ...
   for i=0, vlen-1  do begin
      if(vorder(i) eq k) then begin
         if(istr eq 0) then begin
            ...
            temp=create_struct(names(i),astrc.(i))
      ...
      2: begin
	    ...
	    if q(0) ne -1 then begin
	       ...
	       if j lt size_tmp(2)-1 then goto, test_ts1
            ...
            ts1=tmp_stuff[*,j]
	    if j lt size_tmp(2)-1 then j=j+1
	    test_ts2:
	    q=where(tmp_stuff[*,j] eq dep_fill)
   ...
   if(a.(i).var_notes ne 'ListImage') then begin
      ...
      if(lab(0) ne '') then begin
         ...
         if(z(0) ne -1) then $
                ...
                print, depend1, ' not found (ListImage section of LIST_mystruct).'
...
status=0
printf, unit, format='(15x,"************************************")'
printf, unit, format='(15x,"****  RECORD VARYING VARIABLES  ****")'
printf, unit, format='(15x,"************************************")'
printf, unit, format='(15x)'
...
for i=0L, ntags-5 do begin
   ...
   ;if(a.(i).var_notes eq 'ListImage') then begin 
   if(a.(i).var_notes eq 'ListImage') and $
     ((a.(i).var_type eq 'data') or (a.(i).var_type eq 'support_data')) then begin 
      ...
      if(zz(0) ne -1) then dep2_cmd='dep2=a.(zz(0)).dat' else $
               print, depend2, ' not found (ListImage section of LIST_mystruct).'
      if(zz(0) ne -1) then frm2_cmd='frm2=a.(zz(0)).format' else $
               print, depend2, ' not found (ListImage section of LIST_mystruct).'
      ;dep1_cmd='dep1=a.'+depend1+'.dat'
      ;frm1_cmd='frm1=a.'+depend1+'.format'
      ;dep2_cmd='dep2=a.'+depend2+'.dat'
      ;frm2_cmd='frm2=a.'+depend2+'.format'
      ...
      frm1=ln1+'a'+len1
      frm2=ln2+'a'+len2
      ;      if(frm1 eq '') then frm1=ln1+'a'
      ;      if(frm2 eq '') then frm2=ln2+'a'
...
;
;------------------------------------------------------------------------------------------------
FUNCTION wrt_hybd_strct, a, unit, convar, maxrecs, depend0 
...
case convar of
   0 : begin
       ...
       if(num_data gt maxrecs) then begin
          ...
          printf, unit, format='(a,i6)',text,maxrecs
          printf, unit, format='(a,i6)',text1,dif_rec
          printf, unit, format='(a)',blnk
          status=1
       endif
       printf, unit, format='(15x,"************************************")'
       printf, unit, format='(15x,"*****    GLOBAL ATTRIBUTES    ******")'
       printf, unit, format='(15x,"************************************")'
       printf, unit, format='(15x)'
   ...
   2 : begin
       ...
       ;
       printf, unit, format='(15x,"************************************")'
       printf, unit, format='(15x,"**  NON-RECORD VARYING VARIABLES  **")'
       printf, unit, format='(15x,"************************************")'
       printf, unit, format='(15x)'
   end   ; end case 2
   ;
   ...
   3 : begin
       ...
       ;print,'var type = ',a.(i).var_type
	     ...
	     ;print,'size = ',st_sz
		   ...
		   ;print,'units(inc) 1 = ',units(inc)
             ...
             if(st_sz(0) eq 2) then begin
                num_var=st_sz(1)
		; RCJ 12/02/2003  Commented out the 'if num_var lt 20....'
		;  It doesn't seem to make sense. Will test.
		      ...
		      ; we got errors when inc=0: b was undefined.	 
                ...
                ;endif else begin ; condition where # vars >20 
                   ;; A large number of variables can exceed allowabel 
                   ;;  format lengths etc.
                   ;biglab='2_dim'
                   ;;print, biglab
                   ;for k=0,num_var-1 do begin
                   ;   labels(inc)=label_search(a,st_sz(0),i,k)
		   ;   ;print,'labels(inc) 2 = ',labels(inc)
                   ;   ;units(inc)=a.(i).units
		   ;   units(inc)=unit_search(a,st_sz(0),i,k)
		   ;   ;print,'units(inc) 2 = ',units(inc)
                   ;   inc=inc+1
                   ;endfor
                   ;temp=create_struct(atags(i)+biglab,a.(i).dat(*,0))
                   ;b=create_struct(b,temp)
                ;endelse 
       ...
       if (n_elements(dep1_values) gt 1) then begin
          ...
          printf,unit,format=a.dpform,dep1_values
       endif 
       printf,unit,format=a.uform,units
       ;
       for j=0L, length-1 do begin
   ...
   5 : begin
       ...
       inc=0
       for i=0, ntags-5 do begin
          ;print,'i = ',i,'  varname = ',a.(i).varname,'  ',a.(i).var_type
          if(a.(i).var_type eq 'data') or (a.(i).var_type eq 'support_data') then begin
             labels=''
             units=''
             dep1_values=''
             ...
             if(st_sz(0) eq 2) then begin
                ...
                ;      units=[units,a.(i).units]
                ;      ;temp=create_struct(labels(inc),a.(i).dat(*,0))
                ;      ;temp=create_struct(atags(i)+labels(inc),a.(i).dat(*,0))
  	        ;      uniq = strtrim(string(inc), 2)
                ;      ;temp=create_struct(atags(i)+uniq,a.(i).dat(*,0))
                ;      temp=create_struct(atags(i)+uniq,a.(i).dat(k,0))
                ;      b=create_struct(b,temp)
                ;      inc=inc+1
                ;   endfor   
                ;endif else begin
		      ...
		      unique = strtrim(string(inc), 2)
                      ;temp=create_struct(atags(i)+uniq,a.(i).dat(*,0))
                      ; RCJ 04/11/2001 modified the line above. New line is below.
                      temp=create_struct(atags(i)+unique,a.(i).dat(k,0))
                      b=create_struct(b,temp)
                      inc=inc+1
                   endfor
             ...
             ; create array of depend_2 values, if they exist, to also be listed
             ; RCJ 06/01
             ; exist test is done in dependn_search, if does not exist
             ; return ''
             ...
             if (n_elements(dep2_values) gt 1) then begin
                ...
                if n_elements(tmp_dep2_values) eq n_elements(labels) then begin
                   ...
                   while n_elements(dep2_values)-1 le n_elements(labels)-n_elements(tmp_dep1_values) do begin
                      for kk=0,n_elements(tmp_dep1_values)-1 do begin
             ...
             if(a.(i).var_type eq 'data') or (a.(i).var_type eq 'support_data') then begin
                ...
                if(names(i) eq depend0) then begin
                   ...
                   if(st_sz(0) eq 2) then begin
                      num_var=st_sz(1)
                      ;if(a.(i).var_notes eq 'ListImage') then begin
                      ;   for k=0,num_var-inc-1 do begin
                      ;      ;b.(inc)=a.(i).dat(*,j)
                      ;      b.(inc)=a.(i).dat(k,j)
                      ;      inc=inc+1
                      ;   endfor
...
FUNCTION form_bld, col_sz, label, units, dat_len, dep_col_sz, depend1_labels, $
   dep2_col_sz, depend2_labels, form, shft
 
; Use column size and to build label, unit and data format statements
;
maxlength=max(strlen(depend1_labels)) > max(strlen(depend2_labels)) > strlen(label) 
mintab=fix(dep_col_sz-max(strlen(depend1_labels))) < fix(dep2_col_sz-max(strlen(depend2_labels))) <fix(col_sz-strlen(label))
;
...
nvar=''
ivar=0
for i=0, strlen(format)-1 do begin   
...
function ep_conv, b, depd0, HANDLE=handle
; 
catch, error_status
if(error_status ne 0) then begin
...
if(str_tst(str_tst(0)+1) ne 8) then begin
   ...
   if(rflag(0) ne -1) then begin
      print, a.DATASET
      ;    print, a.ERROR    1/97 read_myCDF change
      print, a.STATUS
      return, 0
   endif
   ;
...
for mega_loop=1, mega.num do begin
   ...
   if(whn) then begin
      ...
      if(namest(incep) eq depend0) then begin
         eptmp=ep_conv(a,depend0,/handle)
   ...
   if(plist gt 3) then begin 
      if reportflag then printf, 1, 'STATUS= Data of 3D or less can be listed. Re-select variables'
   ...
   if keyword_set(TSTART) then begin ; determine datatype and process if needed
	 ...
	 7: begin  ; string
	    if (size(tstart,/tname) eq 'DCOMPLEX') then $
	      start_time=encode_cdfepoch(tstart,/epoch16) else $
   ...
   if keyword_set(TSTOP) then begin ; determine datatype and process if needed
	 ...
	 7: begin   ;string
	    if (size(tstop,/tname) eq 'DCOMPLEX') then $
	       stop_time=encode_cdfepoch(tstop,/epoch16)  else $
...
;         return, 0
   ...
   for i=0,ns_tags-1 do begin
      ...
      form='' ; Default field 
      ; 'EPOCH' or 'EPOCH92' etc.
      if(namest(i) eq depend0) then format='A23' else format=c.(i).format
	 ...
	 (c.(i).cdfrecvary ne 'NOVARY')) then begin 
         ...
         frm_st=data_len(format,c.(i).fillval)
      ...
      ;print,'^^^ ',nogatt,norv,nonrv,no2drv,no3drv,noimg
         if(norv eq 0) then begin
            ...
            sform=form_bld(col_sz, label, units, dat_len, col_sz, label, col_sz, label,form, shft)
		  ...
		  units=unit_search(c,1,i,0)
               ...
               if(st_sz(0) eq 2) then begin
                  ...
                  if(z(0) ne -1) then $
                     ...
                     print, 'Depend_1 not found. This does not necessarily represent a problem (LIST_mystruct).'
      ...
      if(noimg eq 0) then begin
	    ...
	    ;endif
		  ...
		  units=''
                  ...
                  ;col_sz = strlen(label) > strlen(c.(i).units) > dat_len
                  col_sz = strlen(label) > strlen(units) > dat_len
                  ;sform=form_bld(col_sz, label, c.(i).units, dat_len,col_sz,label,form, shft)
                  ;sform=form_bld(col_sz, label, c.(i).units, dat_len,dep_col_sz,depend1_labels,dep_col_sz,depend2_labels,form, shft)
                  sform=form_bld(col_sz, label, units, dat_len,dep_col_sz,depend1_labels,dep_col_sz,depend2_labels,form, shft)
                  ;
                  ;lab_for=lab_for + sform.labv
               	  sform.labv=strmid(sform.labv,0,strlen(sform.labv)-1) ; remove comma
                  ;lab_for=lab_for + strtrim((st_sz(1)-1),2)+'('+sform.labv+'),'
                  lab_for=lab_for + strtrim((st_sz(1)),2)+'('+sform.labv+'),'
                  ;
                  ;unt_for=unt_for + sform.untv
                  sform.untv=strmid(sform.untv,0,strlen(sform.untv)-1) ; remove comma
                  ;unt_for=unt_for + strtrim((st_sz(1)-1),2)+'('+sform.untv+'),'
                  unt_for=unt_for + strtrim((st_sz(1)),2)+'('+sform.untv+'),'
                  ;
                  sform.depv=strmid(sform.depv,0,strlen(sform.depv)-1) ; remove comma
                  ;dep_for=dep_for + strtrim((st_sz(1)-1),2)+'('+sform.depv+'),'
                  dep_for=dep_for + strtrim((st_sz(1)),2)+'('+sform.depv+'),'
                  ;
                  sform.datv=strmid(sform.datv,0,strlen(sform.datv)-1) ; remove comma
                  ;dat_for=dat_for + strtrim((st_sz(1)-1),2)+'('+sform.datv+'),'
                  dat_for=dat_for + strtrim((st_sz(1)),2)+'('+sform.datv+'),'
                  ; Modify sform.datv 
                  ;if(c.(i).var_notes eq 'ListImage') then begin
                     ;lab_for=lab_for + strtrim((st_sz(1)-1),2)+'x,'
                     ;unt_for=unt_for + strtrim((st_sz(1)-1),2)+'x,'
                     ;dep_for=dep_for + strtrim((st_sz(1)-1),2)+'x,'
                     ;sform.datv=strtrim(st_sz(1),2)+'('+form+',1x),'
                  ;endif
                  ;dat_for=dat_for + sform.datv
                  nvar=nvar+1
               endif ; end var_type= data condition
