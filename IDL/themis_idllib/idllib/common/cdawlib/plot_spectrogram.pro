...
;$Header: /home/cdaweb/dev/control/RCS/plot_spectrogram.pro,v 1.82 2003/10/28 20:02:35 kovalick Exp kovalick $
;$Locker: kovalick $
;$Revision: 225 $
;+------------------------------------------------------------------------
...
if (rbegin ge rend) then begin
   print,'ERROR=No data within specified time range.' & return,-1
endif
...
for i=0,num_panels-1 do begin
   ...
   if(igram eq 1) then begin
      ...
      if(i eq (num_panels-1)) then begin
         ;print, xranger(1), tend_save
         ;print, "times(end)", times(n_elements(times)-1)
         if(xranger(1) gt tend_save) then begin ; Pad end
            times=[times,xranger(1)] 
            if (mydata_size(0) eq 2) then begin
               if keyword_set(DEBUG) then print,'Padding back of data...'
               fill = make_array(mydata_size(1),TYPE=mydata_type,VALUE=Zfillval)
               mydata=append_myDATA(fill,mydata) & mydata_size=size(mydata)
      ...
      ; Determine the first and last data time points to be plotted
      rbegin = 0L 
      w = where(times ge xranger(0),wc) & if (wc gt 0) then rbegin = w(0)
      rend = n_elements(times)-1 
      w = where(times le xranger(1),wc) 
      if (wc gt 0) then rend = w(n_elements(w)-1)
      if (rbegin ge rend) then begin
         print,'ERROR=No data within specified time range' & return,-1
      endif
   ...
   if keyword_set(FILLER) then begin
      gaps = -1 & gaps = find_gaps(mytimes,ratio=3.0) & a = size(gaps)
      ...
      while (done eq 0) do begin
         ...
         a = gaps(0) & b=(mytimes(a+1) - mytimes(a)) / resolution & b=long(b)
         if (b gt 0) then begin ; proceed with the gap filling
            ; create array of timing data to fill the gap
            fill = fltarr(b) & c = indgen(b)+1 & d = mytimes(gaps(0))
            fill = fill + ((resolution * c) + d)
            ; plug the fill data into the time data array
            c=mytimes(0:gaps(0)) & d=mytimes(gaps(0)+1:mytimes_size(1)-1)
            mytimes = append_mydata(fill,c) & mytimes = append_mydata(d,mytimes)
            ...
            c=mydata(*,0:gaps(0)) & d=mydata(*,gaps(0)+1:mydata_size(2)-1)
            mydata = append_mydata(fill,c) & mydata = append_mydata(d,mydata)
            ...
            if (myenergy_size(0) eq 2) then begin ; time-varying energy
               ...
               c=myenergy(*,0:gaps(0)) & d=myenergy(*,gaps(0)+1:myenergy_size(2)-1)
               myenergy=append_mydata(fill,c) & myenergy=append_mydata(d,myenergy)
            endif
         endif
         ...
         a = size(gaps) & if (a(0) eq 0) then done = 1
      endwhile
   endif
...
if (a(0) ne -1) then begin
   ...
   if (yvar.scalemax ne '') then begin 
...
if (a(0) ne -1) then begin 
   ; TJK 3/1/2001 add check for if there's a value for the attribute, if 
   ; not don't want to use it.
   if (yvar.validmin ne '') then begin 
...
if (a(0) ne -1) then begin 
   ; TJK 3/1/2001 add check for if there's a value for the attribute, if   
   ; not don't want to use it.
   if (yvar.validmax ne '') then begin 
...
if (keyword_set(AUTO))AND(non_fillcount gt 0) then begin
   ...
   if (a(0) ne -1) then begin 
      ; TJK 3/1/2001 add check for if there's a value for the attribute, if 
      ; not don't want to use it.
      if (zvar.validmin ne '') then begin 
   ...
   if (a(0) ne -1) then begin 
      ; TJK 3/1/2001 add check for if there's a value for the attribute, if 
      ; not don't want to use it.
      if (zvar.validmax ne '') then begin 
   ...
   if (a(0) ne -1) then begin
      ; TJK 3/1/2001 add check for if there's a value for the attribute, if 
      ; not don't want to use it.
      if (zvar.scalemin ne '') then begin 
      ...
      if (desc_str eq 'RPI') then begin
         ...
         spectrogram,transpose(mydata),mytimes,transpose(myenergy),$
	      ...
	      MINVALUE=zmin, MAXVALUE=zmax,$ ;TJK added these   
              /DEVICE,NOERASE=clear_plot,YFILLVAL=Yfillval,/reduce,$
              QUICK=0,CHARSIZE=ycsize,XRANGE=xranger,YRANGE=yranger,$
              CTITLE=zlabel,CSCALE=[zmin,zmax],cCharSize=zcsize,$
