...
pro thm_min2_2,tplot_name_init,period,onemed=onemed
...
for xx=0,dimension1_input-1 do begin
  for yy=0,dimension2_input-1 do begin
    if not keyword_set(onemed) then begin
      if xx mod 100 eq 0 then print,xx,dimension1_input
      for zz=0L+wdw/2,n_elements(data.x)-1-wdw/2 do begin
        data_m [zz,xx,yy]=min(data.y [zz-wdw/2:zz+wdw/2,xx,yy])
      endfor
