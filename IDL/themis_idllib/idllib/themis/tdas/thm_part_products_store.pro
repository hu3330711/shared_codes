...
pro thm_part_products_store,probe=probe,$ ;The requested spacecraft ('a','b','c','d','e','f')
...
print,'*** making FAC coord and gridded data ***'
if strmid(datatype,1,1) eq 'e' and strmid(datatype,3,1) eq 'b' then begin
  anglenum=88
  energynum=31
endif
if strmid(datatype,1,1) eq 'e' and strmid(datatype,3,1) eq 'f' then begin
  anglenum=88
  ;energynum=15
  energynum=31
endif
if strmid(datatype,1,1) eq 'e' and strmid(datatype,3,1) eq 'r' then begin
  anglenum=6
  ;energynum=15
  energynum=31
endif
if strmid(datatype,1,1) eq 's' then begin
  anglenum=64
  energynum=16
endif
...
;**************************
;fac_matrix is a matrix to convert DSL to FAC
;FAC X is earthward. Y is duskward. Z is parallel to B. defined in thm_pgs_make_fac.pro
;fac_matrix[0,*] is the FAC X axis in DSL
;fac_matrix[1,*] is the FAC Y axis in DSL
;fac_matrix[2,*] is the FAC Z axis in DSL
;thm_pgs_do_fac converts the ESA detector directions in DSL (theta and phi) to FAC
;if i eq 13 then stop
if strmid(datatype,1,1) eq 's' then begin
  data_append=fltarr(n_elements(data.data[*,0]),n_elements(clean_data.data[0,*]))
  data_append[0:n_elements(clean_data.data[*,0])-1,*]=clean_data.data
  str_element,clean_data,'data',data_append,/add_replace
  data_append=fltarr(n_elements(data.energy[*,0]),n_elements(clean_data.energy[0,*]))
  data_append[0:n_elements(clean_data.energy[*,0])-1,*]=clean_data.energy
  str_element,clean_data,'energy',data_append,/add_replace
  data_append=fltarr(n_elements(data.theta[*,0]),n_elements(clean_data.theta[0,*]))
  data_append[0:n_elements(clean_data.theta[*,0])-1,*]=clean_data.theta
  str_element,clean_data,'theta',data_append,/add_replace
  data_append=fltarr(n_elements(data.phi[*,0]),n_elements(clean_data.phi[0,*]))
  data_append[0:n_elements(clean_data.phi[*,0])-1,*]=clean_data.phi
  str_element,clean_data,'phi',data_append,/add_replace
  data_append=fltarr(n_elements(data.denergy[*,0]),n_elements(clean_data.denergy[0,*]))
  data_append[0:n_elements(clean_data.denergy[*,0])-1,*]=clean_data.denergy
  str_element,clean_data,'denergy',data_append,/add_replace
  data_append=fltarr(n_elements(data.dtheta[*,0]),n_elements(clean_data.dtheta[0,*]))
  data_append[0:n_elements(clean_data.dtheta[*,0])-1,*]=clean_data.dtheta
  str_element,clean_data,'dtheta',data_append,/add_replace
  data_append=fltarr(n_elements(data.dphi[*,0]),n_elements(clean_data.dphi[0,*]))
  data_append[0:n_elements(clean_data.dphi[*,0])-1,*]=clean_data.dphi
  str_element,clean_data,'dphi',data_append,/add_replace
  data_append=fltarr(n_elements(data.bins[*,0]),n_elements(clean_data.bins[0,*]))
  data_append[0:n_elements(clean_data.bins[*,0])-1,*]=clean_data.bins
  str_element,clean_data,'bins',data_append,/add_replace
  data_append=fltarr(n_elements(data.data[*,0]),n_elements(clean_data.scaling[0,*]))
  data_append[0:n_elements(clean_data.scaling[*,0])-1,*]=clean_data.scaling
  str_element,clean_data,'scaling',data_append,/add_replace

  data_append=data.data*0.0+4.0
  str_element,data,'dt_arr',data_append,/add_replace
  data_append=1.70000e-07
  str_element,data,'dead',data_append,/add_replace
  data_append=3.0297036
  str_element,data,'delta_t',data_append,/add_replace
endif

if i eq 0 then begin
  energynum=n_elements(clean_data.data[*,0])
  anglenum=n_elements(clean_data.data[0,*])
tclean_data_data=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_phi_dsl=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_theta_dsl=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_phi_fac=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_theta_fac=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_dphi_fac=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_dtheta_fac=fltarr(n_elements(time_idx),energynum,anglenum)
tclean_data_data_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_phi_fac_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_theta_fac_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_dphi_fac_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_dtheta_fac_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_gf_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_eff_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_dt_arr_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
tclean_data_energy_grid=fltarr(n_elements(time_idx),energynum,regrid[0]*regrid[1])
endif

if n_elements(tclean_data_data[i,*,*]) eq n_elements(clean_data.data) then begin
tclean_data_data[i,*,*]=clean_data.data
tclean_data_phi_dsl[i,*,*]=clean_data.phi
tclean_data_theta_dsl[i,*,*]=clean_data.theta
spd_pgs_do_fac,clean_data,reform(fac_matrix[i,*,*],3,3),output=clean_data,error=error
tclean_data_phi_fac[i,*,*]=clean_data.phi
tclean_data_theta_fac[i,*,*]=clean_data.theta
tclean_data_dphi_fac[i,*,*]=clean_data.dphi
tclean_data_dtheta_fac[i,*,*]=clean_data.dtheta
clean_data2=clean_data
clean_data2.data=clean_data.dphi
spd_pgs_regrid,clean_data2,regrid,output=clean_data2
spd_pgs_regrid,clean_data,regrid,output=clean_data
;clean_data.dphi=clean_data2.data;otherwise dphi is set constant in thm_pgs_regrid
tclean_data_data_grid[i,*,*]=clean_data.data
tclean_data_phi_fac_grid[i,*,*]=clean_data.phi
tclean_data_theta_fac_grid[i,*,*]=90-clean_data.theta
tclean_data_dphi_fac_grid[i,*,*]=clean_data.dphi
tclean_data_dtheta_fac_grid[i,*,*]=90-clean_data.dtheta

tclean_data_energy_grid[i,*,*]=clean_data.energy
endif
;*************
...
;********************
flag=0
temp=where(outputs_lc eq 'pa',count)
if count eq 1 then flag=1
if flag eq 1 then begin
str_element,data,'eclipse_dphi',success=s
if s eq 0 then str_element,data,'eclipse_dphi',0,/add_replace
store_data,'th'+probe+'_'+datatype+'_clean_data_data',data=clean_data
tclean_data_str={x:times,data:tclean_data_data,energy:clean_data.energy,nenergy:data.nenergy,nbins:data.nbins,geom_factor:data.geom_factor,dead:data.dead,mass:data.mass,charge:data.charge,sc_pot:data.sc_pot,eclipse_dphi:data.eclipse_dphi,units_name:'eflux',units_procedure:'thm_convert_esa_units',phi:tclean_data_phi_dsl,theta:tclean_data_theta_dsl,dphi:clean_data.dphi,dtheta:clean_data.dtheta}
store_data,'th'+probe+'_'+datatype+'_tclean_data_data',data=tclean_data_str
store_data,'th'+probe+'_'+datatype+'_tclean_data_phi_dsl',data={x:times,y:tclean_data_phi_dsl}
store_data,'th'+probe+'_'+datatype+'_tclean_data_theta_dsl',data={x:times,y:tclean_data_theta_dsl}
store_data,'th'+probe+'_'+datatype+'_tclean_data_phi_fac',data={x:times,y:tclean_data_phi_fac}
store_data,'th'+probe+'_'+datatype+'_tclean_data_theta_fac',data={x:times,y:tclean_data_theta_fac}
store_data,'th'+probe+'_'+datatype+'_tclean_data_dphi_fac',data={x:times,y:tclean_data_dphi_fac}
store_data,'th'+probe+'_'+datatype+'_tclean_data_dtheta_fac',data={x:times,y:tclean_data_dtheta_fac}
tclean_data_grid_str={x:times,data:tclean_data_data_grid,energy:tclean_data_energy_grid,nenergy:data.nenergy,nbins:data.nbins,geom_factor:data.geom_factor,dead:data.dead,mass:data.mass,charge:data.charge,sc_pot:data.sc_pot,eclipse_dphi:data.eclipse_dphi,units_name:'eflux',units_procedure:'thm_convert_esa_units',gf:tclean_data_gf_grid,eff:tclean_data_eff_grid,INTEG_T:data.integ_t,delta_t:data.delta_t,dt_arr:tclean_data_dt_arr_grid,regrid:regrid,phi:tclean_data_phi_fac_grid,theta:tclean_data_theta_fac_grid,dphi:tclean_data_dphi_fac_grid,dtheta:tclean_data_dtheta_fac_grid}
store_data,'th'+probe+'_'+datatype+'_tclean_data_data_grid',data=tclean_data_grid_str
store_data,'th'+probe+'_'+datatype+'_tclean_data_phi_fac_grid',data={x:times,y:tclean_data_phi_fac_grid}
store_data,'th'+probe+'_'+datatype+'_tclean_data_theta_fac_grid',data={x:times,y:tclean_data_theta_fac_grid}
store_data,'th'+probe+'_'+datatype+'_tclean_data_dphi_fac_grid',data={x:times,y:tclean_data_dphi_fac_grid}
store_data,'th'+probe+'_'+datatype+'_tclean_data_dtheta_fac_grid',data={x:times,y:tclean_data_dtheta_fac_grid}
store_data,'th'+probe+'_'+datatype+'_tclean_data_pa_spec',data={x:times,y:transpose(pa_spec),v:pa_y},dlim={spec:1}
save,tclean_data_grid_str,filename='th'+probe+'_'+datatype+'_tclean_data_grid_str.sav';needed to create struct without making one-element arrays.
;stop
endif
;********************
