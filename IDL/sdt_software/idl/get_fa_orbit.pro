...
;
; VERSION: @(#)get_fa_orbit.pro	1.23 02/08/07 UCB SSL
;-
...
flg64 = 1
lmdlo = STRING ('idl_orb_get_vectors_3264.so')
if (!VERSION.RELEASE LE '5.4') then begin
    flg64 = 0
    lmdlo = STRING ('idl_orb_get_vectors.so')
endif

if verbose then print, 'shared object is ', libdir + lmdlo
...
status = call_external(libdir + lmdlo, 'idl_orb_get_vectors', $
