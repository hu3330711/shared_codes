...
FUNCTION get_dqi_info, sat, dqi, INDEX=idx, TIME=tselection
   ...
   IF N_ELEMENTS(sat) EQ 0 OR N_ELEMENTS(dqi) EQ 0 THEN BEGIN
      PRINT, '@(#)get_dqi_info.pro	1.7: Input parameters "sat_code" and "dqi_name" must be filled.'
      RETURN, -1
   ENDIF 
   ...
   valid = CALL_EXTERNAL (lmdl, 'getDQIInformation', args)

   IF NOT valid THEN BEGIN
       print, '@(#)get_dqi_info.pro	1.7: error getting DQI info.'
