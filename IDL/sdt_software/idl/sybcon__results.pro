...
flg64 = 1
lmdls5 = STRING ('libsybidl_3264.so')
if (!VERSION.RELEASE LE '5.4') then begin
    flg64 = 0
    lmdls5 = STRING ('libsybidl.so')
endif

self.ncols = call_external(lmdls5, 'sybresults', self.dbproc)
if self.ncols gt 0 then begin
    ...
    structdef = ""
    retval = call_external(lmdls5, 'sybdesc_row', $
                           self.dbproc, $
    ...
    retval = call_external(lmdls5, 'sybbind_row', self.dbproc, $
