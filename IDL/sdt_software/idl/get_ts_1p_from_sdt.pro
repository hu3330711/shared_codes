...
; REVISION HISTORY:
;
;	@(#)get_ts_1p_from_sdt.pro	1.10 01/09/07
...
FUNCTION get_ts_1p_from_sdt, data_type, sat_code, inputTime, INDEX=idx
   ...
   IF N_ELEMENTS (inputTime) EQ 0 THEN BEGIN
      PRINT, '@(#)get_ts_1p_from_sdt.pro	1.10 : An input time argument must be specified'
   ...
   IF secs1970 LE 0.D THEN BEGIN ; format error in date
      PRINT, '@(#)get_ts_1p_from_sdt.pro	1.10 : Invalid input time: ', inputTime
      RETURN, {data_name: 'Null', valid: 0}
   ENDIF
   ...
   len = CALL_EXTERNAL (lmdl, 'getTimePointFromSDTTS',      $
                        long(sat_code),					   $
                        data_type,					   $
                        callYear, 					   $
                        callMonth, 					   $
   ...
   ret = CALL_EXTERNAL (lmdl, 'getTimePointFromSDTTS',       $
                        long(sat_code),					   $
                        data_type,					   $
                        year, 						   $
                        month, 						   $
