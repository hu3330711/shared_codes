...
if keyword_set(orbit) then begin
    ...
    ;; Commented Next section for quicker alternative
    ;;
    ;;com1="awk '/ORBIT: '"""
    ;;com2='"''\t/ {printf("%2s\n%s\n%s\n",$4,$5,$6)}'' '
    ;;orbstring=strtrim(orbit,2)
    ;;command=com1 + orbstring + com2
    ;;
    ;;spawn, command + orbit_file, epoch
    ;;if n_elements(epoch) LT 3 then $
    ;;  message, 'Error getting epoch from orbit file'
    ;;
    ;;n_ep_elems = n_elements(epoch)
    ;;
    ;;yr=strmid(epoch(n_ep_elems-3),2,2)
    ;;doy=epoch(n_ep_elems-2)
    ;;if (!VERSION.RELEASE LE '5.4') then begin
    ;;    ep_hms=str_sep(epoch(n_ep_elems-1),':')
    ;;endif else begin
    ;;    ep_hms=strsplit(epoch(n_ep_elems-1),':',/EXTRACT)
    ;;endelse
    ;;
    ;;tmin=datesec_doy(yr,doy) + ep_hms(0)*3600. + ep_hms(1)*60. + ep_hms(1)
    tmin = get_orbfile_epoch(orbit, orbit_file=orbit_file)
    tmax = tmin + 7990d
endif else begin
    ;; set time1s, time2s to double float secs;  time1, time2 to string
    if keyword_set(time1) AND keyword_set(time2) then begin
        if data_type(time1) EQ 7 then time1s = str_to_time(time1) $
        else time1s = time1 & time1 = time_to_str(time1)
        if data_type(time2) EQ 7 then time2s = str_to_time(time2) $
        else time2s = time2 & time2 = time_to_str(time2)
        middle_time = (time1s + time2s)/2.D
    endif else if keyword_set(xmarkin) then begin
        if data_type(xmarkin) EQ 7 then xmark=str_to_time(xmarkin)
        middle_time = xmark
    endif else middle_time = now_time
    tmin = middle_time - 3995.D	; Do not confuse with keywords
    tmax = middle_time + 3995.D
    ;;epoch = time_to_str(tmin)   ; epoch is unset if orbit is unset
    if tmin LT last_def_epoch then orbit_file=def_orbit_file(0) $
    else begin
        print, 'Switching to predicted orbit file.'
        orbit_file=pre_orbit_file(0)
    endelse
...
; Flag to tell if present time is within plot limits

if (now_time LT tmax) AND (now_time GT tmin) then $
  plot_craft=1 else plot_craft=0

; If Postscript desired set the graphics device to ps.
; If X device in effect, set frame size.
; If Z device in effect, load colors and then set frame size.

if keyword_set(psfile) then begin
    ;; Postscript setup
    ctable = 39
    output = psfile
    popen,output,ctable=ctable,xsize=win, ysize=win, /copy,/port,/color
endif else if !d.name EQ 'Z' then begin
    ;; Z-buffer setup
    if not keyword_set(win) then win = 640
    loadct2, 39
    device, set_resolution=[win, fix(win*1.031)], set_graphics_function=3
endif else begin
    ;; X-window setup
    ;; Allow use of previously set window
    ;; Preserve geometry of plot area
    if not keyword_set(win) then win = 640
    if keyword_set(pc) then retain=2 else retain=1
    if !d.window EQ -1 then window, /free, $
      xsize=fix(win), ysize=fix(win*1.031), retain=retain, $
      title='FAST Magnetic Print Crossing' $
    else begin
        y_size = !d.y_size * .95
        x_size = y_size/1.031
        if x_size GT !d.x_size then begin
            x_size = !d.x_size * .95
            y_size = x_size*1.031
        endif
        x_size_diff = !d.x_size - x_size
        y_size_diff = !d.y_size - y_size
        x_offset = x_size_diff/2
        y_offset = y_size_diff/2
        low_left = [x_offset, y_offset]
        up_right = [x_offset+x_size, y_offset+y_size]
        low_left_norm = convert_coord(low_left, /device, /to_norm)
        up_right_norm = convert_coord(up_right, /device, /to_norm)
        orig_plot_pos = !p.position
        !p.position = reform([low_left_norm(0:1), up_right_norm(0:1)], 4)
    endelse
endelse
...
get_fa_orbit, tmin, tmax, /all, orbit_file=orbit_file, drag_prop=drag_prop, $
...
store_data, 'fa_pos_re', data={x:clock, y:pos_arr}
...
legend = ['LEGEND', $
          ...
          '[] FAST Position Now', $
          '+ FAST Path', $
          'A Apogee (' + string(format='(F5.1)', apogee_lat) + ds + ' lat)', $
          'P Perigee', $
          '- Terminator', $
          '- Auroral Oval']
