...
PRO mhh_gen_file,dict,txt_file,infiles,libfiles,outfile,verbose,title,strict $
   ,crosslink = crosslink,  clturbo = clturbo
;
...
if keyword_set(print_purpose) then if print_purpose eq 2 then purp_delim = '  '
 ...
 for i=0,n_elements(u)-1 do begin
    ...
    PRINTF,final_file,'<LI><A HREF="#DIR_',strtrim(i,2),'">',d,'</A>'
 endfor
 PRINTF,final_file,'</UL><P>'
 PRINTF,final_file,'<HR>'
 PRINTF,final_file,'<H1>Directory Listing of Routines</H1></A>'
 PRINTF,final_file,'<UL>'
 for i=0,n_elements(u)-1 do begin
    d = pathnames(u(i)).dir_name
    PRINTF,final_file,'<A NAME="DIR_',strtrim(i,2),'">'
    PRINTF,final_file,'<H1>',d,'</H1>'
    ...
    PRINTF,final_file,'<br>'
    PRINTF,final_file,'<A HREF="#ROUTINELIST">   [back to top]</A>'
    PRINTF,final_file,'<HR>'
 endfor
 PRINTF,final_file,'</UL><P>'
;stop
...
if keyword_set(crosslink) then begin
  ...
  for cross=0,count-1 do begin
    ...
    newref = '<A href="#'+reference+'">'+reference+'</A>'
...
PRO mk_html_help2, sources, outfile, VERBOSE=verbose,TITLE=title,STRICT=strict $
  ...
  ,FILENAME= outfile2,allfiles=allfiles
 ...
 if not keyword_set(sources) then begin
    sep = ':'    ; default works only with UNIX for now
    if (!VERSION.RELEASE LE '5.4') then begin
        sources = str_sep(!path,sep)
    endif else begin
        sources = strsplit(!path,sep,/EXTRACT)
    endelse
    i = where( strpos(sources,'SCCS') lt 0  )   ; ignore all SCCS directories
    sources = sources(i)
    i = where( strpos(sources,'obsolete') lt 0) ; ignore all obsolete direcs
    sources = sources(i)
    idl_lib = filepath('')
    i = where( strpos(sources,idl_lib) lt 0) ; ignore all idl library direcs
    sources = sources(i)
 endif

if keyword_set(outfile2) then outfile = outfile2
if not keyword_set(outfile) then outfile = 'idl_help.html'
