...
; DESCRIPTION:
;
;	function to get the current position of the SDT RIGHT fiducial
;
;	A structure of the following format is returned:
;
;	   DATA_NAME    STRING    'QTY'           ; Data Quantity name
;	   VALID        INT       1               ; Data valid flag
;	   TYPE         INT       1               ; 1 -> Right fiducial
;	   VALUE        DOUBLE    8.0118726e+08   ; Time of right fid.
;	
; CALLING SEQUENCE:
;
; 	zoom = get_right_from_sdt(data_name, sat_code)
;
;                   or
;
; 	zoom = get_right_from_sdt()
;
; ARGUMENTS:
;
;	data_name      STRING:  The SDT data quantity name.  If not USED, 
;                               then the right fid. of the first quantity
...
FUNCTION get_right_from_sdt, data_name, sat_code
   ...
   len = CALL_EXTERNAL (lmdl, 'getRightFromSDT',             $
                        long(sat_code),					   $
                        data_name,					   $
                        double(lineTime))
