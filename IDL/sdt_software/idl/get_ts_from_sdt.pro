...
; REVISION HISTORY:
;
;	@(#)get_ts_from_sdt.pro	1.15 01/09/07
...
FUNCTION Get_ts_from_sdt, data_name, sat_code, t1=time1, t2=time2, NPTS=npts,    $
   ...
   CASE nkeys OF
      ...
      ELSE: BEGIN 
         PRINT, '@(#)get_ts_from_sdt.pro	1.15 : Illegal combination of keywords'
   ...
   IF t1used THEN BEGIN
      IF NOT KEYWORD_SET(st) AND NOT KEYWORD_SET(en) THEN secs1970_1 = gettime(time1) $
        ...
        ELSE BEGIN ; format error in date1
         PRINT, '@(#)get_ts_from_sdt.pro	1.15 : Invalid input time1: '
         RETURN, {data_name: 'Null', valid: 0}
      ENDELSE 
   ENDIF 
   ...
   IF t2used THEN BEGIN
      IF N_ELEMENTS(time2) GT 0 THEN                       $
        ...
        ELSE BEGIN              ; format error in date2
         PRINT, '@(#)get_ts_from_sdt.pro	1.15 : Invalid input time2'
         RETURN, {data_name: 'Null', valid: 0}
      ENDELSE
   ENDIF 
   ...
   len = CALL_EXTERNAL (lmdl, 'getTimeSeriesFromSDT',        $
                        long(sat_code),					   $
                        data_name,					   $
                        callYear1, 					   $
                        callMonth1, 					   $
   ...
   ret = CALL_EXTERNAL (lmdl, 'getTimeSeriesFromSDT',        $
                        long(new_sat_code),				   $
                        data_name,					   $
                        callYear1, 					   $
                        callMonth1, 					   $
