...
if keyword_set(ebins) then begin
	...
	add_str_element,omni, 'denergy' ,total(dat.denergy(ind,*),1)/count
	add_str_element,omni, 'eff' ,total(dat.eff(ind))/count
	add_str_element,omni, 'ddata'  ,sqrt(omni.data > .7)
	omni.nenergy = 1
...
if ndimen(dat.geom) eq 1 then add_str_element,omni, 'geom'   ,total(dat.geom(ind))
if ndimen(dat.geom) eq 2 then add_str_element,omni, 'geom'   ,total(dat.geom(*,ind),2)
add_str_element,omni, 'denergy' ,total(dat.denergy(*,ind),2)/count
if ndimen(dat.dtheta) eq 1 then add_str_element,omni, 'dtheta' ,total(dat.dtheta(ind))
if ndimen(dat.dtheta) eq 2 then add_str_element,omni, 'dtheta' ,total(dat.dtheta(*,ind),2)
add_str_element,omni, 'ddata'  ,sqrt(omni.data > .7)
omni.nbins = 1
