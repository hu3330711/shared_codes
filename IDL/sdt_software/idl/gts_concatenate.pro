...
if (!VERSION.RELEASE LE '5.4') then begin
    ...
    base_names = [ (reverse(strsplit(datafiles[0], '/', /EXTRACT)))[0] ]
    for b=1, (n_datafiles - 1) do begin
        base_names = [base_names, (reverse(strsplit(datafiles[b], '/', /EXTRACT)))[0] ]
    endfor
endelse
...
for f=0, n_datafiles - 1 do begin
    ...
    if (!VERSION.RELEASE LE '5.4') then begin
        ...
        base_name = (reverse(strsplit(data_file, '/', /EXTRACT)))[0]
        name_fields = strsplit(base_name, '_', /EXTRACT)
    endelse
