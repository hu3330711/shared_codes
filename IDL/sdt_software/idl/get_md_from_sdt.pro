...
; REVISION HISTORY:
;
;	@(#)get_md_from_sdt.pro	1.13 01/09/07
...
FUNCTION Get_md_from_sdt, data_name, sat_code, TIME=inputTime, $
   ...
   IF N_ELEMENTS (inputTime) EQ 0 THEN BEGIN
      PRINT, '@(#)get_md_from_sdt.pro	1.13 : An input time argument must be specified'
   ...
   ELSE IF nkeys GT 1 THEN BEGIN
      PRINT, '@(#)get_md_from_sdt.pro	1.13 : Only one keyword may be used at one time'
      RETURN, {data_name: 'Null', valid: 0}
   ENDIF
   ...
   IF (secs1970 LE 0.D) AND (selectionMode NE 6) AND (selectionMode NE 9)  $
     ...
     THEN BEGIN                 ; format error in date
      PRINT, '@(#)get_md_from_sdt.pro	1.13 : Invalid input time: ', inputTime
      RETURN, {data_name: 'Null', valid: 0}
   ENDIF
   ...
   len = CALL_EXTERNAL (lmdl, 'getTimePointFromSDTMD',       $
                        LONG(sat_code),					   $
                        data_name, 					   $
                        callyear, 					   $
                        callmonth,					   $
   ...
   ret = CALL_EXTERNAL (lmdl, 'getTimePointFromSDTMD',       $
                        LONG(sat_code),					   $
                        data_name, 					   $
                        year, 						   $
                        month, 						   $
