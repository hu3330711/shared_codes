...
;LAST MODIFICATION:     tplot_file.pro   07/02/12
;
;-
...
if keyword_set(save) then begin
   ...
   for i=0,n-1 do begin
     ...
     if data_type(filenames) ne 7 then begin
        timestamp = strmid(time_to_str(min(data.x)),0,10)
...
if keyword_set(restore) then begin
   ...
   if data_type(filenames) ne 7 then $
     filenames = dialog_pickfile(path = dir,/must_exist,get_path=dir,filter='*.tplot') 
