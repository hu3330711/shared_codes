...
if data_type(oldstruct) ne 8 then return
...
if keyword_set(preserve) eq 0 then preserve = 0
...
for i=0,n-1 do begin
   ...
   if index ge 0 then begin
     if not preserve then add_str_element,newstruct,tags(i),oldstruct.(index) $
     else begin
       if find_str_element(newstruct,tags(i)) lt 0 then $     ;if not duplicate
         add_str_element,newstruct,tags(i),oldstruct.(index) $ ;then add tag
       else begin                                             ;if duplicate tag
         if (datatype(newstruct.(index)) eq 7) then $ ;if element is a string, 
           if newstruct.(index) eq "" then $          ;then replace if null
             add_str_element,newstruct,tags(i),oldstruct.(index) $
         else if total(newstruct.(index)) eq 0 then $ ;if element numerical,
           add_str_element,newstruct,tags(i),oldstruct.(index) ;replace if null
       endelse 
     endelse
   endif
endfor
