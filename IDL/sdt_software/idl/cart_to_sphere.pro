...
if ph_0_360 ne 0 then begin
  ...
  if ((ph_0_360 lt 0) and (n_elements(phi) gt 1)) then begin   ;auto range phi
    ...
    mmp  = (ceil(minmax_range(phi,    min=-360,max=360)##subt))(0) ;phi range
    mmtp = (ceil(minmax_range(tmp_phi,min=-360,max=360)##subt))(0) ;tmp range
...
if keyword_set(ph_hist) then begin
  if (dimen1(ph_hist) ne 2) or (data_type(ph_hist) ge 6) then begin
...
if data_type(x(0)) eq 4 then begin ;if x input is float, make angles floats
  theta = float(theta)
  phi = float(phi)
endif 
