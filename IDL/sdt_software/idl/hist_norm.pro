...
sigma[ok] = sqrt(dist[ok] * (1.-dist[ok])/float(norm[ok]-1.))
...
if not keyword_set(no_plot) then begin
    ...
    if keyword_set(show_numbers) then begin
        ...
        xyouts,bins,dist+offset,str(hist),align=0.5,_extra=extra
        xyouts,bins,dist-offset*1.1,str(norm),align=0.5,_extra=extra
