...
if (!VERSION.RELEASE LE '5.4') then begin
    ...
    time_o_day = (strsplit(time_to_str(sec, /msec), '/', /EXTRACT))(1)
    hhmmss = strsplit(time_o_day, ':', /EXTRACT)
    sec_msec = strsplit( strtrim(string(hhmmss(2), format='(F7.4)'),2), '.', /EXTRACT )
endelse
...
if (!VERSION.RELEASE LE '5.4') then begin
    ...
    Xfix = strsplit(strtrim(string(abs(X), format='(D19.7)'), 2), '.', /EXTRACT)
    Yfix = strsplit(strtrim(string(abs(Y), format='(D19.7)'), 2), '.', /EXTRACT)
    Zfix = strsplit(strtrim(string(abs(Z), format='(D19.7)'), 2), '.', /EXTRACT)
endelse
...
if (!VERSION.RELEASE LE '5.4') then begin
    ...
    VXfix = strsplit(strtrim(string(abs(VX), format='(D15.10)'), 2), '.', /EXTRACT)
    VYfix = strsplit(strtrim(string(abs(VY), format='(D15.10)'), 2), '.', /EXTRACT)
    VZfix = strsplit(strtrim(string(abs(VZ), format='(D15.10)'), 2), '.', /EXTRACT)
endelse
