...
;		 2003-09-01 inputTime passed to calibration	EJL/YC
;-

FUNCTION Get_fa_tb_eq, inputTime, START=start, EN=en, ADVANCE=advance,  $
                         RETREAT=retreat, CALIB=calib


      spec=[3,0,2,1]			; species O+ H+ He+ He++
...
; Get samples while dimensions are wrong
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, 'get_fa_tb_eq.pro: Badly formed data.  Getting next sample'
          first = 0
...
;     inputTime = dat.time
      ...
      data_name = 'Tms Burst H+, He++, He+, O+'
      units_name = 'Counts'
      units_procedure = 'convert_tms_units2'
...
;      hdr_time = inputTime
      ...
      data1 = FLTARR(dat.dimsizes(0), dat.dimsizes(2))
      data2 = FLTARR(dat.dimsizes(0), dat.dimsizes(2))

      data = FLTARR(dat.dimsizes(0), nbins, dat.dimsizes(2))
      ...
      ELSE PRINT,"Data reading/timing problem detected by get_fa_tb_eq!"
      ...
      eff = REPLICATE (1., dat.dimsizes(0), nbins, dat.dimsizes(2))
;      eff = FLTARR(dat.dimsizes(0), nbins, dat.dimsizes(2))
      FOR sp = 0, dat.dimsizes(2)-1 DO BEGIN
          data(*, 0, spec(sp)) = data0(*,3,sp) + data0(*,4,sp)
          data(*, 1,spec(sp)) = data0(*,11,sp) +data0(*,12,sp)
          eff0 = FA_TTOF_CALIBRATION(energy, spec(sp), pac, eff_version, inputTime)
;	argument timestring added, summer of 2003, Y. Cao
          eff(*, 0, spec(sp)) = (eff0(*,3) + eff0(*,4))/2.
          eff(*, 1,spec(sp)) = (eff0(*,11) + eff0(*,12))/2.
      ENDFOR

      geom = FLOAT(REPLICATE (2.,dat.dimsizes(0), nbins))
      spin_fract = FLOAT(REPLICATE (1.,dat.dimsizes(0), nbins))
      mass=[0.0104389,2*0.0104389,4*0.0104389,16*0.0104389];mass eV/(km/sec)^2
;      domega = REPLICATE(1., nbins)
     ...
     ; load up the data into IDL data structs
      ...
      RETURN,  {data_name:	data_name, 				      $
                 ...
                 integ_t: 	abs((dat.endTime-dat.time)/dat.dimsizes(0)),  $
                 nbins: 	nbins, 			     	 	      $
