...
FUNCTION Get_fa_tbp_eq, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, 'get_fa_tbp_eq.pro: Badly formed data.  Getting next sample'
          first = 0
...
ENDWHILE
      ...
      data(*,0) = data0(*,3,1) + data0(*,4,1)
      data(*,1) = data0(*,11,1) + data0(*,12,1)



      IF dat.ncomp GT 2 THEN BEGIN
         theta = FLTARR(dat.dimsizes(0), nbins)  
;         FOR i = 0, (nbins-1) DO  theta(*, i) = (.5 + i)*(360./swps(mode))
      ...
      ELSE PRINT,"Data reading/timing problem detected by get_fa_tbp_eq!"
      ...
      eff0 = FA_TTOF_CALIBRATION(energy, spec, pac, eff_version, inputTime)
