...
;     "time_align"
;
; VERSION: @(#)makecdf2.pro	1.12 02/01/01
;-
...
if keyword_set(sktfile) then begin
    ...
    cmd = '$FASTBIN/skeletoncdf -cdf ' + cdffile + ' ' + cmd_overwrite + ' ' + sktfile
    cmd = '(' + cmd + ' ); echo $status'
...
for i = 0, n_tags(data) - 1 do begin
    ...
    if not cdf_var_exists(id, name) then begin
        if ndims ne 0 then begin
            case type of
                1: varid = cdf_varcreate(id,name,dims ne 0,dim=dims,/cdf_uchar,rec_novary=recnovary)
                2: varid = cdf_varcreate(id,name,dims ne 0,dim=dims,/cdf_int2,rec_novary=recnovary)
                3: varid = cdf_varcreate(id,name,dims ne 0,dim=dims,/cdf_int4,rec_novary=recnovary)
                4: varid = cdf_varcreate(id,name,dims ne 0,dim=dims,/cdf_float,rec_novary=recnovary)
                5: varid = cdf_varcreate(id,name,dims ne 0,dim=dims,/cdf_double,rec_novary=recnovary)
            endcase
        endif else begin
            case type of
                1: varid = cdf_varcreate(id,name,zvar=iszvar,/cdf_uchar,rec_novary=recnovary)
                2: varid = cdf_varcreate(id,name,zvar=iszvar,/cdf_int2,rec_novary=recnovary)
                3: varid = cdf_varcreate(id,name,zvar=iszvar,/cdf_int4,rec_novary=recnovary)
                4: varid = cdf_varcreate(id,name,zvar=iszvar,/cdf_float,rec_novary=recnovary)
                5: varid = cdf_varcreate(id,name,zvar=iszvar,/cdf_double,rec_novary=recnovary)
            endcase
        endelse   
    endif
    ...
    if (fill eq 1) and (recnovary eq 0) then begin
        case type of
	    1: fillval = byte(-128)
	    2: fillval = fix(-32768)
    ...
    cdf_varput, id, name, value_shifted, /zvariable
endfor
