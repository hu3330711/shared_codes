...
;-
pro specplot,x,y,z,limits=lim,data=data
...
if n_params() eq 1 then begin
  dim = dimen(x)
  specplot,findgen(dim(0)),findgen(dim(1)),x
  return
endif
...
;   good = where( finite(z1), c )
;   if c eq 0 then message,'No valid Z data.'
;   w = where(z1(good) le 0,c)
;   if c ne 0 then   z1(good(w)) = !values.f_nan
   bad = where(z1 le 0,c)
...
if !d.flags and 1 then begin   ; scalable pixels (postscipt)
   res_dpi = 60.  ; Postscipt defaults to 60 dpi
   str_element,opt,'res_dpi',value=res_dpi
   scale = res_dpi/!d.x_px_cm
endif else scale = 1.
...
if yd eq 1 then begin            ; Typical, y does not vary with time
  ...
  ny = n_elements(y1)
  yp = findgen(nypix)*(!y.crange(1)-!y.crange(0))/(nypix-1) + !y.crange(0)
  ys = interp(findgen(ny),y1,yp)
  if keyword_set(y_no_interp) then  ys = round(ys)

  nxpix = round(scale*npx)
  nx = n_elements(x1)
  xp = findgen(nxpix)*(!x.crange(1)-!x.crange(0))/(nxpix-1) + !x.crange(0)
