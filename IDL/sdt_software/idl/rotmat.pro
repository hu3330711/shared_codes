...
;
; Can't use the CUMULATIVE keyword to TOTAL in the version of IDL that
; we still need to run, so....
;
if float(!version.release) ge 5.3 then begin
    yr_beg = total(inc, /cumulative)
endif else begin
    yr_beg = dblarr(n_elements(inc))
    yr_beg[0] = inc[0]
    for i=1,n_elements(inc)-1 do begin
        yr_beg[i] = yr_beg[i-1] + inc[i]
    endfor
endelse 
