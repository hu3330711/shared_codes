...
;             (default: 1)
; 
;        BOTTOM -> first color index to use (default: 0)
;
...
pro colorbar,color=color,bottom=bottom,ncolors=ncolors, $
      ...
      c_colors=c_colors,c_levels=c_levels,skip=skip,  $
   ...
   if (bottom ge !D.N_COLORS-1) then bottom = !D.N_COLORS-1
   if (n_elements(ncolors) eq 0) then ncolors = !D.N_COLORS-bottom
   if (ncolors gt !D.N_COLORS-bottom) then ncolors = !D.N_COLORS-bottom
   ...
   if (IsDiscrete AND n_elements(C_Levels) eq 0) then $
      Divisions = fix( (n_elements(C_Colors)-1)/Skip ) + 1 
...
if (n_elements(c_levels) gt 0) then print,'#### c_levels=',c_levels
