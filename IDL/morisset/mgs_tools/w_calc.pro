; $Id: w_calc.pro,v 1.11 1999/05/20 16:26:11 mgs Exp $
...
wopstr = [ 'eq','ne','lt','gt','le','ge' ]
  ...
  if (not keyword_set(species)) then title = ''
   ...
   if(ltype) then begin
      x1 = widget_list(dumbase,ysize=9,scr_xsize=mw)
      widget_control,x1,set_value=[ 'index',sstr ]
   ...
   if (ltype) then begin
     x2 = widget_list(dumbase,ysize=9,scr_xsize=mw)
     widget_control,x2,set_value=[ ' ',sstr ]
   ...
   if (ltype) then begin
     wspec = widget_list(lower,ysize=9,scr_xsize=mw)
     widget_control,wspec,set_value=[ 'index',sstr ]
