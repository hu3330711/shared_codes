...
; 
;NOTE:
; This is essentially the same routine as goes_fileconfig
;
;$LastChangedBy: aaflores $
;$LastChangedDate: 2015-06-19 18:59:28 -0700 (Fri, 19 Jun 2015) $
;$LastChangedRevision: 17927 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_0/projects/wind/spedas_plugin/wind_fileconfig.pro $
;--------------------------------------------------------------------------------

pro wind_init_struct,state,struct
...
PRO wind_fileconfig_event, event
  ...
  CASE uval OF
    ...
    'DOON': BEGIN
      ...
      IF event.select EQ 1 then !wind.downloadonly=1 else !wind.downloadonly=0

    END

    'DOOFF': BEGIN

      IF event.select EQ 1 then !wind.downloadonly=0 else !wind.downloadonly=1
    ...
    'RESET': BEGIN
       ...
       if !wind.no_update eq 1 then begin
         ...
         widget_control,state.nu_on_button,set_button=1
       endelse
       if !wind.downloadonly eq 1 then begin
         widget_control, state.do_on_button, set_button=1
       endif else begin
         widget_Control, state.do_off_button, set_button=1
       endelse
...
PRO wind_fileconfig, tab_id, historyWin, statusBar
...
;  historywin->update, msg
  ...
  xmanager, 'wind_fileconfig', master, /no_block
