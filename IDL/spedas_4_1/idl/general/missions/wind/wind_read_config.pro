...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/wind/wind_read_config.pro $
;-
Function wind_config_template
  ...
  Return, ppp
End
Function wind_read_config, header = hhh
  otp = -1
;First step is to get the filename
  dir = wind_config_filedir(/app_query)
...
;check for slashes, add if necessary
  temp_string = strtrim(!wind.local_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !wind.local_data_dir = temporary(temp_string)
  temp_string = strtrim(!wind.remote_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !wind.remote_data_dir = temporary(temp_string)
