...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/rbsp/emfisis/rbsp_emfisis_init.pro $
;-
...
if keyword_set(reset) then !rbsp_emfisis.init=0

if !rbsp_emfisis.init ne 0 then return

!rbsp_emfisis = file_retrieve(/structure_format)    ; force setting of all elements to default values.

rbsp_emfisis_config,no_color_setup=no_color_setup,no_download=no_download


; keywords on first call rbsp_emfisis_init (or /reset) override environment and
; rbsp_emfisis_config
...
servertestfile = '.rbsp_emfisis_master'     ;  This file should only be found on the server. (Do not download this file!)
if file_test(!rbsp_emfisis.local_data_dir+servertestfile) then begin
   !rbsp_emfisis.no_server = 1
;   !rbsp_emfisis.no_download = 1  ; this line is superfluous
endif
...
if cdf_version lt cdf_version_readmin then begin
   print,'Your version of the CDF library ('+cdf_version+') is unable to read RBSP EMFISIS data files.'
...
printdat,/values,!rbsp_emfisis,varname='!rbsp_emfisis'   ;,/pgmtrace

if cdf_version lt cdf_version_writemin then begin
   print,ptrace()
   print,'Your version of the CDF library ('+cdf_version+') is unable to correctly write RBSP EMFISIS CDF data files.'
