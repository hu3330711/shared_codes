...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/rbsp/efw/rbsp_efw_config.pro $
;
;-
pro rbsp_efw_config,caching=caching,colortable=colortable,$
           no_color_setup=no_color_setup,no_download=no_download


;                        RBSP EFW SPECIFIC INITIALIZATION
;====================================================================================


; Location of the data Server:

;!rbsp_efw.use_wget=1   ; uncomment to use experimental wget routine instead of file_http_copy (not recommended)

;Pick your favorite RBSP data server:  (Comment out the others)
; !rbsp_efw.remote_data_dir = 'http://cdpp.cesr.fr/themisdata/'             ; In France
; !rbsp_efw.remote_data_dir = 'http://rhea.iwf.oeaw.ac.at/themisdata/'      ; In Austria, Does not include ground data!
;!rbsp_efw.remote_data_dir = 'http://sprg.ssl.berkeley.edu/data/themis/'   ; use as backup web server
!rbsp_efw.remote_data_dir = 'http://themis.ssl.berkeley.edu/data/rbsp/'

;!rbsp_efw.use_wget=1   ; uncomment to use experimental wget routine
...
!rbsp_efw.local_data_dir = root_data_dir() + 'rbsp/'

;settings in your local rbsp_efw_config.txt file will override the
;defaults
ftest = rbsp_efw_read_config()
If(size(ftest, /type) Eq 8) Then Begin
  !rbsp_efw.local_data_dir = ftest.local_data_dir
  !rbsp_efw.remote_data_dir = ftest.remote_data_dir
  !rbsp_efw.no_download = ftest.no_download
  !rbsp_efw.no_update = ftest.no_update
  !rbsp_efw.downloadonly = ftest.downloadonly
  !rbsp_efw.verbose = ftest.verbose
Endif

; Settings of environment variables can override thm_config
if getenv('RBSP_EFW_DATA_DIR') ne '' then $
   !rbsp_efw.local_data_dir = getenv('RBSP_EFW_DATA_DIR')

if file_test(!rbsp_efw.local_data_dir + '.rbsp_efw_master',/regular) then !rbsp_efw.no_download = 1

if getenv('RBSP_EFW_REMOTE_DATA_DIR') ne '' then $
   !rbsp_efw.remote_data_dir = getenv('RBSP_EFW_REMOTE_DATA_DIR')

;check for slashes, add if necessary, jmm, 2-jul-2007
temp_string = strtrim(!rbsp_efw.local_data_dir, 2)
...
temp_string = strtrim(!rbsp_efw.remote_data_dir, 2)
ll = strmid(temp_string, strlen(temp_string)-1, 1)
If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
...
if keyword_set(no_download) then !rbsp_efw.no_download=1
...
If(!rbsp_efw.verbose Eq 0) Then tplot_options, 'verbose', 0 $ ;turn off default to verbose if !themis.verbose is zero, jmm, 30-sep-2009
