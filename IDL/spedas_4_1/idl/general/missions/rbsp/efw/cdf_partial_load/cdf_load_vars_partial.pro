...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/rbsp/efw/cdf_partial_load/cdf_load_vars_partial.pro $
;
;-
function cdf_load_vars_partial,files,varnames=vars,varformat=vars_fmt,info=info,verbose=verbose,all=all, $
...
vars=''
info = 0
dprint,dlevel=4,verbose=verbose,'$Id: cdf_load_vars_partial.pro 13281 2013-10-08 19:43:36Z peters $'
...
;        if vb ge 4 then printdat,/pgmtrace,vars,width=200
        ...
        if keyword_set(spdf_dependencies) then begin  ; Get all the variable names that are dependents
            depnames = ''
            edepnames = strarr(n_elements(vars))
            for i=0,n_elements(vars)-1 do begin
                ...
                edepnames[i] = cdf_var_atts(id,vi.num,zvar=vi.is_zvar,'DEPEND_0',default='')
                ndim = vi.ndimen
            ...
            if keyword_set(edepnames) then edepnames=edepnames[[where(edepnames)]]
            edepnames = edepnames[uniq(edepnames,sort(edepnames))]            
;            if vb ge 4 then printdat,/pgmtrace,depnames,width=200

       endif
    endif

    if keyword_set(edepnames) then cdf_partial_epoch_finder,id,edepnames,epoch_indices
...
;            if   vi.recvary or 1  then begin ;disabling logic that does nothing, pcruce@igpp.cula.edu
             ...
             thisedepname = cdf_var_atts(id,vi.num,zvar=vi.is_zvar,'DEPEND_0',default='')
             whichepoch = where(edepnames eq thisedepname, whichepoch_cnt)
             if whichepoch_cnt eq 0 then begin
                if (vi.datatype eq 'CDF_EPOCH') or (vi.datatype eq 'CDF_EPOCH16') or (vi.datatype eq 'CDF_TIME_TT2000') then begin
                   whichepoch = where(edepnames eq vi.name, whichepoch_cnt)
                endif
             endif
             ...
             if n_elements(number_records) ne 0 then begin
               ...
               endelse
             endelse
             if whichepoch_cnt ne 0 then begin
                numrec = epoch_indices[whichepoch,1] - epoch_indices[whichepoch,0] + 1
                record = epoch_indices[whichepoch,0]
             endif
...
;            endif else numrec = 0
            ...
            if numrec gt 0 then begin
                ...
                if vi.recvary then begin
                    if (vi.ndimen ge 1 and ((n_elements(record) eq 0) or keyword_set(whichepoch_cnt))) then begin
