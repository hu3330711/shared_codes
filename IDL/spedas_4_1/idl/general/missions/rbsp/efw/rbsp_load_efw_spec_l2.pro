...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/rbsp/efw/rbsp_load_efw_spec_l2.pro $
;-

pro rbsp_load_efw_spec_l2,probe=probe, trange=trange, $
  ...
  rbsp_efw_init
  dprint,verbose=verbose,dlevel=4,'$Id: rbsp_load_efw_spec_l2.pro 28942 2020-07-27 20:36:11Z aaronbreneman $'
  ...
  for s=0,n_elements(p_var)-1 do begin
     rbspx = 'rbsp'+ p_var[s]
     rbsppref = rbspx + '/l2/spec/'


     ;Find out what files are online
     format = rbsppref + '/YYYY/'+rbspx+'_efw-l2_spec_YYYYMMDD_v*.cdf'
     ...
     if is_string(tns) then begin
        ...
        options, /def, tns, code_id = '$Id: rbsp_load_efw_spec_l2.pro 28942 2020-07-27 20:36:11Z aaronbreneman $'
