...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/rbsp/efw/rbsp_efw_read_config.pro $
;-
Function rbsp_efw_config_template
  ...
  Return, ppp
End
Function rbsp_efw_read_config, header = hhh
  otp = -1
;First step is to get the filename
  dir = rbsp_efw_config_filedir(/app_query)
  If(dir[0] Ne '') Then Begin
;Is there a trailing slash? Not for linux or windows, not sure about Mac
    ll = strmid(dir, strlen(dir)-1, 1)
    If(ll Eq '/' Or ll Eq '\') Then filex = dir+'rbsp_efw_config.txt' $
    Else filex = dir+'/'+'rbsp_efw_config.txt'
;Does the file exist?
    If(file_search(filex) Ne '') Then Begin
      template = rbsp_efw_config_template()
