...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/general/missions/rbsp/spacecraft/rbsp_spice_config.pro $
;
;-
pro rbsp_spice_config,caching=caching,colortable=colortable,$
           no_color_setup=no_color_setup,no_download=no_download


;                        RBSP SPICE SPECIFIC INITIALIZATION
;====================================================================================


; Location of the data Server:

;!rbsp_spice.use_wget=1   ; uncomment to use experimental wget routine instead of file_http_copy (not recommended)

;Pick your favorite RBSP data server:  (Comment out the others)
; !rbsp_spice.remote_data_dir = 'http://cdpp.cesr.fr/themisdata/'             ; In France
; !rbsp_spice.remote_data_dir = 'http://rhea.iwf.oeaw.ac.at/themisdata/'      ; In Austria, Does not include ground data!
;!rbsp_spice.remote_data_dir = 'http://sprg.ssl.berkeley.edu/data/themis/'   ; use as backup web server
!rbsp_spice.remote_data_dir = 'http://themis.ssl.berkeley.edu/data/rbsp/'

;!rbsp_spice.use_wget=1   ; uncomment to use experimental wget routine
...
!rbsp_spice.local_data_dir = root_data_dir() + 'rbsp/'

;settings in your local rbsp_spice_config.txt file will override the
;defaults
ftest = rbsp_spice_read_config()
If(size(ftest, /type) Eq 8) Then Begin
  !rbsp_spice.local_data_dir = ftest.local_data_dir
  !rbsp_spice.remote_data_dir = ftest.remote_data_dir
  !rbsp_spice.no_download = ftest.no_download
  !rbsp_spice.no_update = ftest.no_update
  !rbsp_spice.downloadonly = ftest.downloadonly
  !rbsp_spice.verbose = ftest.verbose
Endif

; Settings of environment variables can override thm_config
if getenv('RBSP_EFW_DATA_DIR') ne '' then $
   !rbsp_spice.local_data_dir = getenv('RBSP_EFW_DATA_DIR')

if file_test(!rbsp_spice.local_data_dir + '.rbsp_spice_master',/regular) then !rbsp_spice.no_download = 1

if getenv('RBSP_EFW_REMOTE_DATA_DIR') ne '' then $
   !rbsp_spice.remote_data_dir = getenv('RBSP_EFW_REMOTE_DATA_DIR')
...
if keyword_set(no_download) then !rbsp_spice.no_download=1
...
if !prompt eq 'IDL> ' then !prompt = 'RBSP_EFW> '
...
If(!rbsp_spice.verbose Eq 0) Then tplot_options, 'verbose', 0 $ ;turn off default to verbose if !themis.verbose is zero, jmm, 30-sep-2009
