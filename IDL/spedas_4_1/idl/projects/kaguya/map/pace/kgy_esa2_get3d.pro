...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/kaguya/map/pace/kgy_esa2_get3d.pro $
;-

function kgy_esa2_get3d, time, index=index, cntcorr=cntcorr, sabin=sabin, infoangle=infoangle, verbose=verbose,gettimes=gettimes
...
if n_elements(esa2_header_arr) eq 0 then begin
   dprint,dlevel=0,verbose=verbose,'No esa2 data are stored'
   return, {project_name:'Kaguya MAP/PACE',data_name:'ESA-S2',valid:0}
endif
if size(index,/type) eq 0 then begin
   ...
   i = where( esa2_header_arr[*].index eq theindex , i_cnt )
   if i_cnt ne 1 then begin 
      dprint,dlevel=0,verbose=verbose,'kgy_esa2_get3d: INVALID INDEX'
      return, {project_name:'Kaguya MAP/PACE',data_name:'ESA-S2',valid:0}
   endif
endelse
...
delta_t = esa2_header_arr[i].time_resolution/1000.  ;- time resolution

ram = esa2_header_arr[i].svs_tbl

case esa2_header_arr[i].type of
   '00'XB: begin                ;- TYPE00 32x16x64
      ...
      gfactor[enesq[*],polsq[*],*] = esa2_info_str.gfactor_16x64[ram,*,*,*]

      if not keyword_set(INFOangle) then begin
         energy[enesq[*],*,*] = $ ;- keV -> eV
            rebin(reform(esa2_fov_str.ene[ram,*]),32,16,64) * 1000.
         theta[enesq[*],polsq[*],*] $
            = -rebin(reform(esa2_fov_str.pol16[ram,*,*]),32,16,64)
         phi[*,*,*] = transpose(rebin(esa2_fov_str.az64[*],64,16,32))
      endif else begin
         energy[enesq[*],polsq[*],*] = $
            esa2_info_str.ene_16x64[ram,*,*,*] * 1000. ;- keV -> eV
         theta[enesq[*],polsq[*],*] = esa2_info_str.pol_16x64[ram,*,*,*]
         phi[enesq[*],polsq[*],*] = esa2_info_str.az_16x64[ram,*,*,*]
      ...
      ii = where( esa2_type00_arr.index eq theindex )
      cnt = float(esa2_type00_arr[ii].cnt)
      ...
      if not keyword_set(cntcorr) then begin
         ...
         event = float(esa2_type00_arr[ii].event)
         trash = float(esa2_type00_arr[ii].trash)
   ...
   '01'XB: begin
      ...
      gfactor[enesq[*],polsq[*],*] = esa2_info_str.gfactor_4x16[ram,*,*,*]

      if not keyword_set(INFOangle) then begin
         energy[enesq[*],*,*] = $ ;- keV -> eV
            rebin(reform(esa2_fov_str.ene[ram,*]),32,4,16) * 1000.
         theta[enesq[*],polsq[*],*] $
            = -rebin(reform(esa2_fov_str.pol4[ram,*,*]),32,4,16)
         phi[*,*,*] = transpose(rebin(esa2_fov_str.az16[*],16,4,32))
      endif else begin
         energy[enesq[*],polsq[*],*] = $
            esa2_info_str.ene_4x16[ram,*,*,*] * 1000. ;- keV -> eV
         theta[enesq[*],polsq[*],*] = esa2_info_str.pol_4x16[ram,*,*,*]
         phi[enesq[*],polsq[*],*] = esa2_info_str.az_4x16[ram,*,*,*]
         idx_0 = where( gfactor eq 0 , idx_0_cnt )
         if idx_0_cnt gt 0 then bins[idx_0] = 0
      endelse
      ...
      ii = where( esa2_type01_arr.index eq theindex )
      cnt = float(esa2_type01_arr[ii].cnt)
      idx_nan = where( cnt eq uint(-1) , idx_nan_cnt)
      if idx_nan_cnt gt 0 then cnt[idx_nan] = !values.f_nan
      if not keyword_set(cntcorr) then begin
         ccnt = cnt
         ...
         event = float(esa2_type01_arr[ii].event)
         trash = float(esa2_type01_arr[ii].trash)
         idx_nan = where( event eq uint(-1) , idx_nan_cnt)
         if idx_nan_cnt gt 0 then event[idx_nan] = !values.f_nan
         idx_nan = where( trash eq uint(-1) , idx_nan_cnt)
         if idx_nan_cnt gt 0 then trash[idx_nan] = !values.f_nan
...
dat = { $
      ...
      sensor:long(esa2_header_arr[i].sensor), $ ;- sensor ID
      mode:long(esa2_header_arr[i].mode), $   ;- data mode
      mode2:long(esa2_header_arr[i].mode2), $ ;- sub-data mode
      type:long(esa2_header_arr[i].type), $   ;- data type
      ver:long(esa2_header_arr[i].ver), $     ;- data version
      svg:long(esa2_header_arr[i].svg_tbl), $ ;- IMA IEA
      sva:long(esa2_header_arr[i].sva_tbl), $
      svs:long(esa2_header_arr[i].svs_tbl), $ ;- RAM?
      obs:long(esa2_header_arr[i].obs_tbl), $

      nbins:long(esa2_header_arr[i].pol_step*esa2_header_arr[i].az_step), $
      nenergy:long(esa2_header_arr[i].ene_step), $
      ntheta:long(esa2_header_arr[i].pol_step), $
      nphi:long(esa2_header_arr[i].az_step), $
      nevent:long(esa2_header_arr[i].event_step), $
      ntrash:long(esa2_header_arr[i].trash_step), $
