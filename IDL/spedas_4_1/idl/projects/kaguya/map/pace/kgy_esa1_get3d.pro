...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/kaguya/map/pace/kgy_esa1_get3d.pro $
;-
...
if keyword_set(gettimes) then begin
   times = $
      time_double( string(esa1_header_arr[*].yyyymmdd,format='(i8.8)') $
                   +string(esa1_header_arr[*].hhmmss,format='(i6.6)'), $
...
start_time = $
   time_double( string(esa1_header_arr[i].yyyymmdd,format='(i8.8)') $
                +string(esa1_header_arr[i].hhmmss,format='(i6.6)'), $
                tformat='YYYYMMDDhhmmss' )
