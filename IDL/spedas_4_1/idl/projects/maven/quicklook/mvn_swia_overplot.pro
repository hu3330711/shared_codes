...
; mvn_swia_overplot
;PURPOSE:
; MAVEN PFP SWIA Quicklook Plot
;CALLING SEQUENCE:
; mvn_swia_overplot, date = date, time_range = time_range, $
...
;           'mvn_swia_qlook_start_time_end_time.png'
...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/maven/quicklook/mvn_swia_overplot.pro $
...
varlist = ['mvn_swia_temps', 'mvn_swia_imons', 'mvn_swia_vmon_fixed', $
           'mvn_swia_voltages', 'mvn_swis_en_counts', 'mvn_swifs_en_counts', $
           'mvn_swics_ph_counts', 'mvn_swim_density', $
           'mvn_swim_velocity', 'mvn_swim_temperature']
...
If(keyword_set(makepng)) Then Begin
    ...
    fname = pdir+mvn_qlook_filename('swi', tr, _extra=_extra)
