...
;-
FUNCTION mvn_sta_get_ca,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

common mvn_ca,get_ind,all_dat

if n_elements(get_ind) eq 0 then begin
	if keyword_set(times) then return,0
	dat = 	{project_name:'MAVEN',valid:0}
	print,' ERROR - mvn ca data not loaded'
	return,dat
...
;		header:			all_dat.header[ind],			$
		...
		nenergy: 		nenergy, 							$
		energy: 		reform(all_dat.energy[swp_ind,*,*],nenergy,nbins), 		$
		denergy: 		reform(all_dat.denergy[swp_ind,*,*],nenergy,nbins),  		$

		nbins: 			nbins,	 							$
;		bins: 			replicate(1.,nenergy)#all_dat.bins, 				$
		bins: 			all_dat.bins, 							$
		ndef:			all_dat.ndef,							$
		nanode:			all_dat.nanode,							$

		theta: 			reform(all_dat.theta[swp_ind,*,*],nenergy,nbins),  		$
		dtheta: 		reform(all_dat.dtheta[swp_ind,*,*],nenergy,nbins),  		$
		phi: 			reform(all_dat.phi[swp_ind,*,*],nenergy,nbins),  		$
		dphi: 			reform(all_dat.dphi[swp_ind,*,*],nenergy,nbins),  		$
		domega: 		reform(all_dat.domega[swp_ind,*,*],nenergy,nbins),  		$

		gf: 			reform(all_dat.gf[swp_ind,*,*,att_ind],nenergy,nbins),		$
		eff: 			reform(all_dat.eff[eff_ind,*,*],nenergy,nbins),	 		$
		...
		nmass:			nmass,								$
		mass: 			all_dat.mass, 							$
		mass_arr: 		reform(all_dat.mass_arr[swp_ind,*,*],nenergy,nbins), 		$
		tof_arr: 		reform(all_dat.tof_arr[mlut_ind,*,*],nenergy,nbins), 		$
		twt_arr: 		reform(all_dat.twt_arr[mlut_ind,*,*],nenergy,nbins), 		$
