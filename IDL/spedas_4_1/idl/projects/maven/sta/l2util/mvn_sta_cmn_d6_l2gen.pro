...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/maven/sta/l2util/mvn_sta_cmn_d6_l2gen.pro $
;-
...
;First, global attributes
  global_att = {Acknowledgment:'None', $
                Data_type:'l2_d6-events>Level 2 Events data', $
                Data_version:'0', $
...
; Here are variable names, type, catdesc, and lablaxis, from the SIS
  rv_vt =  [['EPOCH', 'TT2000', 'UTC time from 01-Jan-2000 12:00:00.000 including leap seconds), one element per ion distribution (NUM_DISTS elements)', 'TT2000'], $
            ...
            ['TDC_1', 'INTEGER', 'Time-to-Digital Converter (TDC) 1, time of flight from TA-TC.', 'tdc_1'], $
            ['TDC_2', 'INTEGER', 'Time-to-Digital Converter (TDC) 2, time of flight from TB-TD.', 'tdc_2'], $
            ['TDC_3', 'INTEGER', 'Time-to-Digital Converter (TDC) 3, position from TA-TB.', 'tdc_3'], $
            ['TDC_4', 'INTEGER', 'Time-to-Digital Converter (TDC) 4, position from TC-TD.', 'tdc_4'], $
            ['EVENT_CODE', 'INTEGER', 'Event code: bit0-5: A-first, B-first, A-pulse valid, B-pulse valid, C-pulse valid, D-pulse valid', 'ev_code'], $
            ['CYCLESTEP', 'INTEGER', 'Cycle step (0-1023) during a data accumulation that the event happened', 'cyclestep'], $
            ['ENERGY', 'FLOAT', 'Sensor energy during the event', 'energy'], $
...
;No need for lablaxis values here, just use the name
  nv_vt = [['PROJECT_NAME', 'STRING', 'MAVEN'], $
           ...
           ['TDC1_CONV', 'FLOAT', 'Conversion factor for TDC1 '], $
           ['TDC2_CONV', 'FLOAT', 'Conversion factor for TDC2'], $
           ['TDC3_CONV', 'FLOAT', 'Conversion factor for TDC3'], $
           ['TDC4_CONV', 'FLOAT', 'Conversion factor for TDC4'], $
           ['TDC1_OFFSET', 'FLOAT', 'Offset to subtract from TDC1 before multiplying by TDC1_CONV to get TOF in nanoseconds'], $
           ['TDC2_OFFSET', 'FLOAT', 'Offset to subtract from TDC2 before multiplying by TDC1_CONV to get TOF in nanoseconds'], $
           ['TDC3_OFFSET', 'FLOAT', 'Offset to subtract from TDC3 before multiplying by TDC1_CONV to get POS in nanoseconds'], $
...
;change data to float from double
     if(vj eq 'energy') then dvar = float(dvar) 
...
;Create filename and call cdf_save_vars.
  ...
  ext = strcompress(strlowcase(cmn_dat.apid), /remove_all)+'-events'
