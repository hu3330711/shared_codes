...
;  $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/themis/spacecraft/particles/ESA/thm_esa_energy_extrapolate.pro $
;-

pro thm_esa_energy_extrapolate,dist_data,add_energy,lin_energy=lin_energy,lin_counts=lin_counts,lsquadratic=lsquadratic,quadratic=quadratic,spline=spline,lsquares=lsquares,error=error,bin_select=bin_select
  ...
  for i = 0,n_elements(dist_data)-1 do begin
    ...
    if n_elements(str_energy_dim) eq 1 then begin
      str_energy_dim = [str_energy_dim,1]
    endif
   
    ;use number of energies to detect retrace bin
    ;the variable below is used as the index to the first bin that will be included in the output
    if str_energy_dim[0] ne 32 then begin
      retrace=0
    endif else begin
      retrace=1
    endelse
    ...
    ;create source target, remove retrace
    energy_target = str_template.energy[retrace:str_energy_dim[0]-1,0]
    energy_source = str_template.energy[retrace:str_energy_dim[0]-1,*]
   
    energy_target = [reverse(add_energy[bsort(add_energy)]),energy_target]
    ...
    ;if n_elements(str_energy_dim) eq 2 then begin
      new_energy_arr = energy_target#(dblarr(str_energy_dim[1])+1)
      new_denergy_arr = [(abs(deriv(energy_target)))[0:n_elements(add_energy)-1],str_template.denergy[retrace:str_energy_dim[0]-1,0]]#(dblarr(str_energy_dim[1])+1) ;very quick and very dirty delta-energies
    ;endif else if n_elements(str_energy_dim) eq 1 then begin
    ;  new_energy_arr = energy_target
    ;  new_denergy_arr = [(abs(deriv(energy_target)))[0:n_elements(add_energy)-1],str_template.denergy[retrace:str_energy_dim[0]-1,0]]
    ;endif else begin
    ;  dprint,dlevel=1,"ERROR: unexpected energy dimensions"
    ;  return
    ;endelse
    ...
    for j=0,n_elements(tag_names)-1 do begin
      ...
      if strlowcase(tag_names[j]) ne 'energy' and $
         ...
         if array_equal(tag_dimen,str_energy_dim) || (str_energy_dim[1] eq 1 && str_energy_dim[0] eq tag_dimen[0] && n_elements(tag_dimen) eq 1) then begin
    ...
    for j = 0,n_elements(out_data)-1 do begin
       ...
       for k=0,n_elements(tag_names)-1 do begin
        ...
        if strlowcase(tag_names[k]) ne 'energy' and $
           ...
           if array_equal(tag_dimen,new_energy_dim) then begin
             ...
             new_data_arr = dblarr(new_energy_dim)
             new_data_arr[n_elements(add_energy):new_energy_dim[0]-1,*] = (*dist_data[i])[j].(k)[retrace:str_energy_dim[0]-1,*]
             new_data_arr[0:n_elements(add_energy)-1,*] = (dblarr(n_elements(add_energy))+1)#(*dist_data[i])[j].(k)[retrace,*]
      ...
      source_data = (*dist_data[i])[j].data[retrace:str_energy_dim[0]-1,*]
      source_bins = (*dist_data[i])[j].bins[retrace:str_energy_dim[0]-1,*]        
