...
function cl_ui_get_user_selections, state, event
...
; START EVENT HANDLER
;----------------------
pro cl_ui_load_data_event,event
  ...
  if is_string(uval) then begin
    case uval of
      ...
      'ADD': begin
        ...
        loadstruc = cl_ui_get_user_selections(state, event)
        ...
        if size(loadstruc, /type) NE 8 then begin
          ...
          cl_ui_load_data_load_pro, $
           ...
           callSeqStruc = { type:'loadapidata', $
                            subtype:'cl_ui_load_data_load_pro', $
                            loadStruc:loadStruc, $
...
pro cl_ui_load_data,tabid,loadedData,historyWin,statusBar,treeCopyPtr,timeRangeObj,callSequence,loadTree=loadTree,timeWidget=timeWidget
  ...
  topBase = Widget_Base(tabid, /Row, /Align_Top, /Align_Left, YPad=1,event_pro='cl_ui_load_data_event') 
  ...
  instrumentArray = ['aspoc','cis','dwp','edi','efw','fgm','peace','rapid','staff','wbd','whisper']
