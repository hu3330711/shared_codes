...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/cluster/common/cl_init.pro $
;-

pro cl_init, reset=reset, local_data_dir=local_data_dir, remote_data_dir=remote_data_dir,$
  ...
  if not keyword_set(exists) then begin; if !cluster does not exist
    defsysv,'!cluster', def_struct
  endif

  if keyword_set(reset) then !cluster.init=0

  if !cluster.init ne 0 then begin
    ...
    str_element, !cluster, 'mirror_data_dir', success=mirror_available
    if mirror_available && !cluster.mirror_data_dir ne '' then !cluster.mirror_data_dir = spd_addslash(!cluster.mirror_data_dir)
    return
  endif
  ...
  !cluster = def_struct; force setting of all elements to default values.
  !cluster.preserve_mtime = 0
  
  cl_config,no_color_setup=no_color_setup; override the defaults by local config file

  cl_set_verbose ;propagate verbose setting into tplot_vars
  
  ; keywords on first call to cl_init (or /reset) override environment and
  ; cl_config
  if keyword_set(local_data_dir) then begin 
    !cluster.local_data_dir = spd_addslash(local_data_dir)
  endif
  if keyword_set(remote_data_dir) then begin
    !cluster.remote_data_dir = spd_addslash(remote_data_dir)
  endif
  if keyword_set(mirror_data_dir) then begin
    !cluster.mirror_data_dir = spd_addslash(mirror_data_dir)
  endif
