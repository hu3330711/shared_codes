...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/mms/common/cdf/mms_cdf_load_vars.pro $
;
;-
function mms_cdf_load_vars,files,varnames=vars,varformat=vars_fmt,info=info,verbose=verbose,all=all, $
                ...
                if vi.recvary then begin
                    if (vi.ndimen ge 1 and n_elements(record) eq 0) then begin
                        if numrec eq 1 then begin
                            ...
                            if (~is_array(size(/dimensions,value)) && size(/dimensions,value) eq 0) then begin
                                value = reform(value)
                            endif else begin
                                value = reform(/overwrite,value, [1,size(/dimensions,value)] )
                                ; the following broke on L2 FPI files with only a single time stored in them
                                ;value = transpose(reform(/overwrite,value, [1,size(/dimensions,value)] ))  ; Special case for variables with a single record
                            endelse
