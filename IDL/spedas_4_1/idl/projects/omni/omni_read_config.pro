...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_4_1/projects/omni/omni_read_config.pro $
;-
...
;check for slashes, add if necessary
  temp_string = strtrim(!omni.local_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !omni.local_data_dir = temporary(temp_string)
  temp_string = strtrim(!omni.remote_data_dir, 2)
  ll = strmid(temp_string, strlen(temp_string)-1, 1)
  If(ll Ne '/' And ll Ne '\') Then temp_string = temp_string+'/'
  !omni.remote_data_dir = temporary(temp_string)
